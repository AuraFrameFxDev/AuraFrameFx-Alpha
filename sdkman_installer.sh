...
# echo_failed_command prints the last executed command and its exit code if the previous command failed.
echo_failed_command() {
    local exit_code="$?"
	if [[ "$exit_code" != "0" ]]; then
...
# infer_platform detects the current operating system and hardware architecture and echoes a platform identifier string.
