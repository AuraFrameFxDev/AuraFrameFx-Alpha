...
TEST_F(LanguageIdL2cJniTest, MultipleLanguageDetection) {
    ...
    for (const auto& testCase : testCases) {
        ...
        EXPECT_TRUE(isValidLanguageCode(result)) 
            << "Invalid language code for: " << testCase.description;
        EXPECT_EQ(result, testCase.expected_language)
            << "Wrong language detected for: " << testCase.description;
    }
}
...
TEST_F(LanguageIdL2cJniTest, MixedLanguageText) {
    ...
    EXPECT_TRUE(result == "en" || result == "fr" || result == "es");
}
...
TEST_F(LanguageIdL2cJniTest, ConfidenceScoring) {
    ...
    for (const auto& testCase : testCases) {
        ...
        EXPECT_GE(confidence, 0.0) << "Confidence should be non-negative for: " << testCase.description;
        EXPECT_LE(confidence, 1.0) << "Confidence should not exceed 1.0 for: " << testCase.description;
        EXPECT_GE(confidence, testCase.expected_min_confidence) 
            << "Confidence too low for: " << testCase.description;
    }
}
...
TEST_F(LanguageIdL2cJniTest, MemoryManagement) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        mock_env->ReleaseStringUTFChars(jstr, chars);
    }
}
...
TEST_F(LanguageIdL2cJniTest, SupportedLanguages) {
    ...
    for (const auto& lang : commonLanguages) {
        EXPECT_TRUE(isValidLanguageCode(lang)) 
            << "Language code should be valid: " << lang;
    }
}
