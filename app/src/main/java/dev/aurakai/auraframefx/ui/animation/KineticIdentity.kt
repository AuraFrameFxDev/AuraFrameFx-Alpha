...
object KineticIdentity {
    ...
    /**
     * Creates an infinite repeatable animation spec that smoothly pulses a float value between its initial state and a target value.
     *
     ...
     * @param targetValue The maximum float value reached during the pulse.
     ...
     *
     * The `intensity` parameter is included for API consistency but has no effect on the animation.
     *
     * @param durationMillis Duration of the animation in milliseconds.
     * @return A tween animation spec with linear easing suitable for glitch or shake animations.
     */
    ...
    /**
     * Returns an animation spec that adds a dramatic pause before performing the specified action animation, using a dramatic cubic bezier easing.
     *
     * The total duration is the sum of the pause and the action animation durations. If the action spec is not a tween, the standard duration is used for the action segment.
     *
     ...
     * @return An animation spec combining a pause and an action with dramatic easing.
     */
...
/**
     ...
     *
     * The resulting animation spec has a total duration equal to the specified delay plus the original animation's duration. For spring specs, the duration is estimated as 1000 ms; for other types, a standard duration is used.
     *
...
/**
     ...
     *
     * @param repeatMode The repeat mode for repetition, either [RepeatMode.Restart] or [RepeatMode.Reverse]. Defaults to [RepeatMode.Restart].
...
/**
 * Returns a tween animation spec with the specified easing if the original spec is a tween; otherwise, returns the original animation spec unchanged.
 *
 * @param easing The easing function to apply when the animation spec is a tween.
 * @return A tween animation spec with the given easing, or the original animation spec if not a tween.
 */
