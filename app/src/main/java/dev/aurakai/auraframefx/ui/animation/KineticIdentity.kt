...
object KineticIdentity {
    ...
    /**
     * Returns an infinite repeatable animation spec that smoothly pulses between the initial value and [targetValue].
     *
     * The animation cycles in reverse mode, creating a continuous breathing or pulsing effect.
     *
     * @param durationMillis Duration of one complete pulse cycle in milliseconds. Defaults to 2000 ms.
     * @param targetValue The maximum value reached during the pulse. Defaults to 1.1.
     ...
     * @param intensity Intended shake intensity (not applied in the returned spec).
     * @return An animation spec suitable for use in glitch or shake animations.
     */
     ...
     *
     * The resulting animation waits for the specified pause duration, then executes the action using a dramatic cubic bezier easing. The total duration is the sum of the pause and the action's duration (if available).
     *
     * @param pauseDurationMillis Duration of the initial pause in milliseconds. Defaults to 500 ms.
     * @param actionSpec Animation spec for the action following the pause. Defaults to `DaringEnter`.
     * @return An animation spec combining the pause and dramatic action.
     */
...
/**
     ...
     *
     * The resulting animation spec has a total duration equal to the specified delay plus the estimated duration of the original animation. For spring specs, the duration is estimated as 1000 ms; for other types, a standard duration is used.
     *
...
/**
 * Returns a copy of this animation spec with the given easing if it is a tween; otherwise, returns the original spec.
 *
