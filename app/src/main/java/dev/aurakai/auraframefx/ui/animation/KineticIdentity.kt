...
object KineticIdentity {
    ...
    /**
     * Creates an infinite repeatable animation spec that smoothly transitions between the initial value and [targetValue], producing a continuous breathing or pulsing effect.
     *
     * @param durationMillis Duration of one complete pulse cycle in milliseconds. Default is 2000 ms.
    ...
    /**
     * Returns an animation spec that introduces a pause before performing a dramatic action with cubic bezier easing.
     *
     * The animation first delays for the specified pause duration, then executes the action. The total duration is the sum of the pause and the action's duration (using the action spec's duration if it is a tween, or a standard duration otherwise).
     *
...
/**
     ...
     *
     * The resulting tween's duration is the sum of the delay and the estimated duration of the original animation spec. For spring-based specs, the duration is estimated as 1000 ms; for tweens, the original duration is used; for other types, a standard duration is applied.
     *
 ...
 * @return A tween animation spec with the provided easing, or the original animation spec if not a tween.
 */
