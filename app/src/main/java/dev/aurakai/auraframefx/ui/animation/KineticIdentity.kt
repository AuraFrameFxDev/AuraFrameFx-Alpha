...
object KineticIdentity {
     ...
     *
     * The animation transitions a value between its initial state and [targetValue], repeating in reverse mode for a continuous in-and-out motion.
     *
     ...
     * @return An [AnimationSpec] suitable for glitch or shake-style animations.
     */
     ...
     *
     * The animation first holds for the specified pause duration, then performs the action using a dramatic cubic bezier easing. The total duration is the sum of the pause and the action's duration.
     *
     * @param pauseDurationMillis The length of the pause before the action begins, in milliseconds.
     * @param actionSpec The animation spec for the action that follows the pause.
     * @return An animation spec combining the pause and the action with dramatic easing.
     */
...
/**
 * Returns a tween animation spec with the specified easing if this animation spec is a tween; otherwise, returns the original spec unchanged.
 *
 * @param easing The easing function to use if creating a new tween animation spec.
 * @return A tween animation spec with the given easing, or the original animation spec if it is not a tween.
 */
