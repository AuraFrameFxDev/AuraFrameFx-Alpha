...
object KineticIdentity {
     ...
     *
     * The animation cycles a value between its starting point and [targetValue] using linear easing and reverse repeat mode, creating a continuous in-and-out motion.
     *
     ...
     * @return An [AnimationSpec] for producing glitch or shake visual effects.
     */
     ...
     *
     * The animation waits for the specified pause duration, then executes the action using a dramatic cubic bezier easing. The total duration is the sum of the pause and the action's duration (estimated if not a tween).
     *
     * @param pauseDurationMillis Duration of the pause before the action, in milliseconds.
     * @param actionSpec The animation spec for the action after the pause.
...
/**
     ...
     *
     * The resulting animation spec has a total duration equal to the specified delay plus the estimated duration of the original animation. For spring specs, the duration is estimated as 1000 ms; for tween specs, the actual tween duration is used; otherwise, a standard duration is applied.
     *
...
/**
     * Returns an infinite repeatable animation spec based on this animation, repeating indefinitely with the specified repeat mode.
     *
     * @param repeatMode Whether the animation restarts or reverses direction on each repeat. Defaults to [RepeatMode.Restart].
     * @return An [InfiniteRepeatableSpec] that repeats this animation endlessly.
     */
