...
object KineticIdentity {
    ...
    /**
     * Returns an infinite repeatable animation spec that smoothly pulses a float value between its initial and target value using linear easing.
     *
     * The animation reverses direction each cycle, creating a continuous breathing or pulsing effect.
     *
     * @param durationMillis Duration of one pulse cycle in milliseconds. Defaults to 2000 ms.
     * @param targetValue The peak value reached during the pulse. Defaults to 1.1.
     ...
     * @return An [AnimationSpec] for animating float values with a linear glitch or shake effect.
     */
    ...
    /**
     * Returns an animation spec that adds a pause before starting the given action animation, combining both durations with a dramatic cubic bezier easing.
     *
     * @param pauseDurationMillis Duration of the initial pause in milliseconds before the action animation begins.
     * @param actionSpec The animation spec to execute after the pause; if not a tween, a standard duration is used.
