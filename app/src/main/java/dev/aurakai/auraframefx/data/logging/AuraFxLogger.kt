...
class AuraFxLogger @Inject constructor(
     ...
     *
     * Scans for files with the log filename prefix and deletes those whose last modified timestamp is beyond the retention threshold.
     */
     ...
     *
     * After calling this method, no additional log entries will be written or processed by the logger.
     */
    fun shutdown() {
     ...
     *
     * Creates parent directories if they do not exist. Appends to the file if specified; otherwise, overwrites it.
     *
     * @param filePath Relative path to the file within the internal files directory.
