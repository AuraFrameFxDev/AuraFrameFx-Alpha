...
class AuraFxLogger @Inject constructor(
    ...
    /**
         * Asynchronously logs an error-level message with the given tag and optional throwable.
         *
     ...
     *
     * @return A map where each key is a log filename (matching the log filename prefix) and the value is its content, with the newest files listed first.
     */
     ...
     *
     * Scans the log directory for files with the designated log filename prefix and deletes those whose last modified time exceeds the retention threshold.
     */
    ...
    /**
     * Cancels all ongoing logging and maintenance operations, disabling further log processing and file writes.
     *
     * After calling this method, the logger will no longer handle or persist any log entries.
     */
    fun shutdown() {
     ...
     *
     * Creates parent directories if they do not exist. Can either append to or overwrite the file based on the `append` flag.
     *
