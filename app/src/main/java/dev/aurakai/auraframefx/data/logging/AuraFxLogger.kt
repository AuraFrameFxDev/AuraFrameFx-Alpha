...
class AuraFxLogger @Inject constructor(
     ...
     *
     * Scans for files with the log filename prefix and deletes those whose last modified time exceeds the retention threshold.
     */
    ...
    /**
     * Returns a map of all log filenames to their contents, sorted with the newest files first.
     *
     * @return A map where each key is a log filename and each value is the corresponding file's content.
     */
    ...
    /**
     * Removes all log files from the internal logs directory that use the designated log filename prefix.
     *
