...
class AuraFxLogger @Inject constructor(
    ...
    /**
         * Asynchronously logs a debug-level message and optional throwable to both Android Logcat and the current day's internal log file.
         *
...
>>>>>>> pr458merge
    /**
      * Asynchronously logs an error-level message to both Android Logcat and the current day's internal log file.
      *
      * @param tag Identifier for the source of the log message.
...
>>>>>>> pr458merge
     ...
     *
     * Only files with the log filename prefix are considered. Files whose last modified time exceeds the retention threshold are deleted.
     */
    ...
    /**
     * Shuts down the logger, cancelling all ongoing logging and maintenance coroutines.
     *
     * After this method is called, no further log entries will be processed or written.
     */
    /**
     * Cancels all ongoing logging and maintenance coroutines, shutting down the logger's coroutine scope.
     */
    fun shutdown() {
...
>>>>>>> pr458merge
     ...
     * Creates parent directories if they do not exist. Appends to the file if `append` is true; otherwise, overwrites the file.
     *
     * @param filePath Relative path within the app's internal storage.
     * @param content The text content to write.
