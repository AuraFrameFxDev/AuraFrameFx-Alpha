...
class ContextManager @Inject constructor(
     ...
     *
     * Searches for related memories based on the interaction content, selects the most appropriate agent, and computes a priority value. Returns an `EnhancedInteractionData` object containing the original interaction details along with the enriched context, agent recommendation, and priority.
     *
     ...
     *
     * Extracts and stores memories from the interaction if the confidence score is high, enabling future retrieval and learning.
     */
     ...
     *
     * Creates an `Insight` with the provided request, response, complexity, and extracted pattern descriptors, and adds it to the insight store. Triggers asynchronous learning processing after every 10 recorded insights.
     *
    ...
    /**
     * Builds a formatted summary string containing metadata and current system states for the given context.
     *
    ...
    /**
     * Extracts and stores a conversation entry as a memory if its confidence score exceeds 0.8.
     *
     * Captures user input, agent response, agent type, and relevant tags for high-confidence interactions to support future retrieval.
     *
