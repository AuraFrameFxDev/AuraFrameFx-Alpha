...
class TrinityCoordinatorService @Inject constructor(
    ...
    /**
     * Analyzes an AI request and routes it to the appropriate AI persona or fusion mode, emitting one or more responses as a Flow.
     *
     * Determines the optimal processing strategy based on request content, complexity, and ethical considerations. Supports routing to Kai, Aura, Genesis fusion, ethical review, or parallel processing with synthesis. Emits a failure response if the system is not initialized or if an error occurs.
     *
     ...
     *
     * Initiates the given fusion type in the Genesis persona, optionally providing additional context, and emits an `AgentResponse` indicating whether activation was successful.
     *
     ...
     *
     * If retrieval fails, the map contains an "error" entry with the failure message.
     *
    ...
    /**
     * Determines the optimal routing strategy and optional Genesis fusion type for a given AI request.
     *
     * Analyzes the request's content for ethical concerns, fusion triggers, and relevant keywords to select between routing to Kai, Aura, Genesis fusion, parallel processing, or ethical review. If ethical concerns are detected (unless skipped), the request is routed for ethical review. Otherwise, the function identifies if a specific Genesis fusion type is required or selects the most suitable AI persona(s) based on the request's intent.
     *
     ...
     *
     * @return `true` if any ethical concern keyword is present in the message; `false` otherwise.
     */
