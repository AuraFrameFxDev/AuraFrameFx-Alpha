...
class MemoryManager @Inject constructor(
     ...
     *
     * The returned items are sorted by descending timestamp. Only items whose agent matches the query's agent filter (if provided) are included.
     *
     ...
     *
     * Filters memory items to those whose timestamps fall within the maximum chain length duration from the current time, sorts them by most recent, and limits the result to the maximum chain length.
     *
    ...
    /**
     * Updates the memory statistics state flow with the current total item count, number of recent items, and total memory content size.
     */
