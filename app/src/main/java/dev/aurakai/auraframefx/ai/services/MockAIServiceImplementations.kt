...
>>>>>>> pr458merge
     ...
     * Returns a Flow that emits a single mock `AgentResponse` echoing the input query with a confidence score of 1.0.
     *
     * @return A Flow containing one mock response for the given request.
     */
    override fun processRequestFlow(request: AiRequest): Flow<AgentResponse> { // Added from Agent interface
...
override fun getName(): String? = "MockCascade"
    /**
 * Returns the constant agent type `AgentType.CASCADE` for this mock AI service.
 *
 * @return The fixed agent type representing the mock Cascade AI service.
 */
