...
data class VertexAIConfig(
     ...
     *
     * Checks that required string fields (`projectId`, `location`, `endpoint`, `modelName`) are not blank, and that numeric parameters such as `timeoutMs`, `maxRetries`, `maxConcurrentRequests`, `defaultTemperature`, and `defaultTopP` are within valid ranges.
     *
     * @return A list of error messages describing invalid or missing configuration fields, or an empty list if the configuration is valid.
     */
     ...
     *
     * @return The base endpoint URL for the Vertex AI API.
     */
    ...
    /**
     * Constructs the full URL for the content generation API endpoint for the configured model.
     *
     * The returned URL incorporates the base endpoint, API version, project ID, location, and model name.
     *
     * @return The complete URL to invoke content generation for the specified model.
     */
    ...
    /**
     * Returns a copy of this configuration with parameters adjusted for production environments.
     *
     * The returned configuration enables safety filters, increases retry count and timeout, activates caching, metrics, and logging, and sets the log level to "WARN" for enhanced reliability and security in production.
     *
     ...
     *
     * The development configuration disables safety filters and caching, reduces retries and timeout for quicker feedback, and enables detailed logging and metrics for easier debugging.
     *
