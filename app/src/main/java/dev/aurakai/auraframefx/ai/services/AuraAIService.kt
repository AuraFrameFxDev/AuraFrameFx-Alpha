...
class AuraAIService @Inject constructor(
    ...
    /**
     * Prepares the AI models and creative context features required by AuraAIService.
     *
    ...
    /**
     * Handles an AI request by generating a creative text response or emitting an error as a coroutine flow.
     *
     * Validates the request for security, generates a creative response using the provided query and context, and emits an `AgentResponse` containing the result or error details.
     *
     * @param request The AI request with query and optional context.
     ...
     *
     * Validates the prompt for security, enhances it with creative context, and generates text with high creativity settings. The output is post-processed before being returned.
     *
    ...
    /**
     * Converts a list of memories into a bullet-pointed string, including each memory's content and relevance score.
     *
     * @param memories The memories to summarize.
     * @return A formatted string with each memory on a separate line.
     */
     ...
     * @return A prompt string instructing AI to generate Kotlin code for the specified animated component.
     */
 ...
 *
 * @return A map with the keys "name" (service name), "type" (agent type), and "service_implemented" (Boolean indicating if the service is implemented).
 */
