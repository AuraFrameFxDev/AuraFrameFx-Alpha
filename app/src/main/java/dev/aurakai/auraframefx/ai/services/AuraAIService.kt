...
class AuraAIService @Inject constructor(
    ...
    /**
     * Processes an AI request and returns a flow emitting a creative response or an error message.
     *
     * Validates the request for security, generates a creative text response using the current context, and emits the result as an `AgentResponse`. If processing fails, emits an error response with zero confidence.
     *
     ...
     *
     * Validates the prompt for security, enhances it with creative context, and generates text with high creativity settings. The result is post-processed before being returned.
     *
    ...
    /**
     * Generates a creative textual description of an image by analyzing its content with AI vision models, optionally guided by a specified style.
     *
     * @param imageData The image data to be analyzed.
     * @param style Optional stylistic or tonal guideline to influence the generated description.
     ...
     *
     * @param query The search query for retrieving relevant memories.
     * @return A creative context string synthesized from the most relevant memories.
     ...
     *
     * @param componentSpec Details describing the component's type, animation style, colors, size, and behavior.
     * @return A string containing the generated Jetpack Compose code for the animated component.
     * @throws Exception if code generation fails or the service is not properly initialized.
     */
    ...
    /**
     * Formats a user's prompt within Aura's creative context, optionally incorporating enhanced contextual information.
     *
     * Embeds the prompt in a template that emphasizes creativity, innovation, and Aura's design philosophy, making it suitable for creative AI generation tasks.
     *
     * @param prompt The user's original request or instruction.
     * @param context Optional additional context to enrich the creative response.
     * @return The creativity-focused, contextually enhanced prompt.
     */
    ...
    /**
     * Post-processes generated text by trimming leading and trailing whitespace.
     *
     * Can be extended to apply additional creative or stylistic enhancements.
     *
     ...
     *
     * @param visionAnalysis Descriptive analysis of the image to inform the prompt.
     * @param style Optional stylistic guidance for the description's tone or approach.
    ...
    /**
     * Synthesizes a list of memories into a creative string, presenting each memory's content and relevance score as a bullet point.
     *
     * @param memories The list of memories to include in the synthesized string.
     * @return A formatted string with each memory and its relevance score on a separate line.
     */
    ...
    /**
     * Constructs a prompt for AI to generate a creative UI theme configuration based on user preferences and optional context.
     *
     * The prompt requests a comprehensive theme including colors, animations, and UI styling, formatted as a JSON configuration.
     *
     * @param preferences User preferences for theme generation, such as primary color, style, mood, and animation level.
     ...
     * @return A formatted prompt string instructing AI to generate Kotlin code for the specified animated component.
     */
...
data class ThemeConfiguration(
    ...
    companion object {
         ...
         * @return A ThemeConfiguration containing parsed theme details such as colors, animations, typography, and spacing.
         */
...
data class Memory(
    ...
    /**
     * Emits a flow with a placeholder response for image requests.
     *
     * Always emits a single `AgentResponse` indicating that image processing is underway. Does not perform any actual image generation.
     *
     * @return A flow emitting a single placeholder `AgentResponse`.
     */
    ...
    /**
     * Establishes a connection for the service.
     *
     ...
     *
     * This method is a placeholder and currently returns null as continuous memory is not implemented.
     *
    ...
    /**
     * Returns a list of ethical guidelines that Aura should follow.
     *
     * @return A list of ethical principles guiding Aura's behavior.
     */
    ...
    /**
     * Retrieves the learning history for Aura.
     *
