...
class ContextManager @Inject constructor(
     ...
     *
     * The new chain is associated with the specified root context, initial content, agent, and optional metadata (all metadata values are stored as strings). The chain is added to the set of active contexts, and context statistics are updated.
     *
     ...
     *
     * Filters active context chains by agent if specified, sorts them by most recent update, and limits the results according to configuration and query parameters. If no matching chains are found, a new chain is created using the query string as both the root and current context. Related chains are further filtered by minimum relevance and limited in number.
     *
    ...
    /**
     * Updates the context statistics, including total number of chains, count of recently active chains, length of the longest chain, and the timestamp of the update.
     *
