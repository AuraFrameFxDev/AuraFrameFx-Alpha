...
class ContextManager @Inject constructor(
     ...
     *
     * Initializes a context chain using the specified root context, initial content, agent, and optional string-based metadata. The new chain is added to the active context registry, and context statistics are updated.
     *
     ...
     *
     * Updates the chain's current context, history, agent-to-context mapping, and last updated timestamp. Throws an exception if the specified chain does not exist.
     *
     ...
     * @return The matching ContextChain, or null if no chain exists for the provided ID.
     */
     ...
     *
     * Filters active context chains by agent if provided, sorts them by most recent update, and applies relevance and length constraints. If no matching chains are found, returns a new chain initialized with the query string as both the root and current context.
     *
     ...
     *
     * Recalculates the total number of chains, the count of recently active chains, the length of the longest chain, and the timestamp of the last update.
     */
