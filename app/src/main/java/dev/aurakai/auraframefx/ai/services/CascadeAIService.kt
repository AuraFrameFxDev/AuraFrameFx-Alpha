...
override fun getType(): AgentType = AgentType.CASCADE
    ...
    /**
     * Routes an AI request to the appropriate internal handler based on its type and returns a flow emitting agent responses.
     *
     * Recognized request types ("state", "context", "vision", "processing") are delegated to specialized handlers. For unrecognized types, emits a default response indicating a basic query.
     *
     ...
     *
     * The returned [AgentResponse] contains the original query and context, with a fixed confidence score of 0.75.
     *
     * @param request The AI request to respond to.
     * @param context Additional context to embed in the response.
    ...
    /**
     * Emits a flow with a single response summarizing the agent's current internal state.
     *
     * The response includes all key-value pairs from the internal state as a formatted string and assigns a confidence score of 1.0.
     *
    ...
    /**
     * Collects the first response from both Aura and Kai AI services for a context-type request and emits a single aggregated AgentResponse.
     *
     * The emitted response concatenates the content from both services and averages their confidence scores.
     *
     ...
     *
     * @return A flow with one [AgentResponse] stating "Processing vision state..." and a confidence score of 0.9.
     */
     ...
     *
     * @return A flow emitting one AgentResponse with a message about state transition processing and a confidence score of 0.9.
     */
     ...
     *
     * @return A flow with one [AgentResponse] containing a retrieval message and a confidence score of 0.95.
     */
     ...
     *
     * @return A map with the agent's name ("name"), type ("type"), and a boolean indicating if the service is implemented ("service_implemented").
     */
