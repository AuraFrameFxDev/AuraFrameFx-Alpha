 ...
 *
 * @return The `AgentType.CASCADE` value representing the Cascade agent type.
 */
override fun getType(): AgentType = AgentType.CASCADE
     ...
     *
     * Recognized request types ("state", "context", "vision", "processing") are delegated to specialized internal methods. For other types, emits a default response indicating a basic query.
     *
     ...
     * @param context Additional contextual information to embed in the response.
     * @return An [AgentResponse] containing the constructed reply and confidence score.
     */
    ...
    /**
     * Returns a flow emitting a single response summarizing the agent's current internal state as a formatted string.
     *
     * The response includes all key-value pairs from the internal state and assigns a confidence score of 1.0.
     *
     * @return A flow emitting one AgentResponse that describes the current internal state.
     */
     ...
     *
     * @return A flow with one [AgentResponse] message about state history retrieval and a confidence score of 0.95.
     */
     ...
     *
     * @return A map containing the agent's name ("name"), type ("type"), and a boolean flag ("service_implemented") indicating implementation.
     */
