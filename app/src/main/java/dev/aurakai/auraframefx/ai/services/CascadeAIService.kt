 ...
 *
 * @return The agent type, indicating this is a Cascade agent.
 */
override fun getType(): AgentType = AgentType.CASCADE
     ...
     *
     * For recognized request types ("state", "context", "vision", "processing"), delegates to specialized internal methods. Emits a default response for unrecognized types.
     *
     ...
     *
     * The response includes the original query and the supplied context, with a fixed confidence score of 0.75.
     *
     * @param request The AI request to respond to.
     * @param context Additional context to include in the response.
     * @return An [AgentResponse] containing the composed response and confidence score.
     */
    ...
    /**
     * Emits a flow containing a single response that lists all current internal state key-value pairs as a formatted string.
     *
     * The response content presents the agent's complete state and is returned with a confidence score of 1.0.
     *
    ...
    /**
     * Emits a flow containing a single response indicating that a vision-related request is being processed.
     *
     ...
     *
     * @return A flow with one AgentResponse describing state transition processing and a confidence score of 0.9.
     */
    ...
    /**
     * Emits a flow containing a response that indicates the agent is retrieving its state history.
     *
     ...
     *
     * @return A map with capability details: "name" (agent name), "type" (agent type), and "service_implemented" (true if implemented).
     */
