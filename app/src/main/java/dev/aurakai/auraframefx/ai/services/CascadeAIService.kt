 ...
 */
    ...
    /**
     * Routes an AI request to the appropriate internal handler based on the "type" key in the request's context map and emits agent responses as a flow.
     *
     * Delegates processing to specialized internal methods for "state", "context", "vision", or "processing" types. Emits a default response flow for unrecognized or missing types.
     *
     * @param request The AI request containing a context map with a "type" key used for routing.
     * @return A flow emitting agent responses corresponding to the routed request type.
     */
    ...
    /**
     * Returns a direct response to an AI request, combining the request's query and the provided context.
     *
     * The response content includes both the query and context, with a fixed confidence score of 0.75.
     *
     ...
     *
     * The response lists all key-value pairs stored in the agent's internal state map, with a confidence score of 1.0.
     *
