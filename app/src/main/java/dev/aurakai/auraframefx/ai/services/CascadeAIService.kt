...
override fun getType(): AgentType = AgentType.CASCADE
    ...
    /**
     * Processes an AI request by routing it to the appropriate internal handler based on its type and emits agent responses as a flow.
     *
     * Recognized request types ("state", "context", "vision", "processing") are delegated to specialized internal flows. For unrecognized types, emits a default response indicating a basic query was received.
     *
    ...
    /**
     * Returns a direct response to an AI request by combining the request's query with the provided context.
     *
    ...
    /**
     * Emits a flow containing a single response summarizing the agent's current internal state.
     *
     * The response presents all key-value pairs in the state as a formatted string and assigns a confidence score of 1.0.
     *
    ...
    /**
     * Collects the first response from both AuraAIService and KaiAIService for a context-type AI request and emits a single AgentResponse.
     *
     * The emitted response contains concatenated content from both services and the average of their confidence scores.
     *
     ...
     *
     * The emitted response contains a fixed message and a confidence score of 0.9.
     */
     ...
     *
     * @return A map with keys "name", "type", and "service_implemented" describing the agent.
     */
