...
override fun getType(): AgentType = AgentType.CASCADE
    ...
    /**
     * Processes an AI request and emits agent responses as a flow based on the request's type.
     *
     * Routes the request to specialized handlers for "state", "context", "vision", or "processing" types. Emits a default response for unrecognized types.
     *
    ...
    /**
     * Emits a response containing the current internal state as a formatted string.
     *
     * @return A flow emitting a single AgentResponse with the agent's state and full confidence.
     */
    ...
    /**
     * Processes a context-type AI request by aggregating the first responses from both Aura and Kai AI services.
     *
     * Emits a single response that combines the content from both services, with the confidence set as the average of their individual confidences.
     *
     * @return A flow emitting the aggregated agent response.
     */
     ...
     *
     * The response content is a fixed message and the confidence is set to 0.95.
     *
     * @return A flow emitting a single [AgentResponse] about state history retrieval.
     */
