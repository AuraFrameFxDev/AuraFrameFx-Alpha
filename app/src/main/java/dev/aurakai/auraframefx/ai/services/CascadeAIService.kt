...
override fun getType(): AgentType = AgentType.CASCADE
     ...
     *
     * Handles "state", "context", "vision", and "processing" request types with specialized response flows. For other types, emits a default acknowledgment response with moderate confidence.
     *
    ...
    /**
     * Returns a direct response combining the AI request's query and the provided context with a fixed confidence score of 0.75.
     *
     * The response content concatenates the query and context for immediate, non-streaming processing.
     *
    ...
    /**
     * Emits a flow with a single response summarizing the agent's current internal state as key-value pairs.
     *
    ...
    /**
     * Emits a flow with a single response indicating that vision state processing is underway.
     *
     * The response contains a message about processing the vision state and a confidence score of 0.9.
     */
     ...
     *
     * The map includes the agent's name ("Cascade"), type ("CASCADE"), and a flag indicating the service is implemented (true).
     *
     * @return A map with capability details for the agent.
     */
