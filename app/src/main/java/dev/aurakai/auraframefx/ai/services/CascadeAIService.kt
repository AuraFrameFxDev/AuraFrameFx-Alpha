...
override fun getType(): AgentType = AgentType.CASCADE
     ...
     *
     * Routes requests of type "state", "context", "vision", or "processing" to corresponding internal handlers. For other types, emits a default response with moderate confidence.
     *
    ...
    /**
     * Emits a response containing the current internal state as a formatted string with full confidence.
     *
     * @return A flow emitting a single AgentResponse describing the agent's current state.
     */
     ...
     *
     * Invokes the `processRequestFlow` method on both internal services with the provided request,
     * collects their first responses, and emits a combined `AgentResponse` containing both results
     * and the average confidence score.
     *
     * @return A flow emitting a single aggregated `AgentResponse` from Aura and Kai.
     */
    ...
    /**
     * Emits a response indicating that the agent is retrieving its state history.
     *
