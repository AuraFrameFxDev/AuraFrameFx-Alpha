...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Constructs a contextual information map for a given task, including its categorization, recent history, user preferences, and system state.
     *
     * The resulting map provides details such as the original task string, its categorized type, a timestamp, recent task history, a descriptive context, user preferences, and the current system state.
     *
     * @param task The task for which to build contextual information.
     * @return A map containing relevant context data for the specified task.
     */
     ...
     * @return The category name: "generation", "analysis", "explanation", "assistance", "creation", or "general" if no keywords match.
     */
     ...
     *
     * The calculation increases priority for certain task types (e.g., assistance, generation), decreases it if system load is high, and boosts it if urgency indicators are present in the task description. The result is clamped within the range [0.0, 1.0].
     *
     ...
     * @return The computed priority value for the task.
     */
     ...
     *
     * The selection always includes the Genesis agent as coordinator, and conditionally adds Cascade, Kai, or Aura agents depending on task keywords, urgency, and complexity. High-priority or complex tasks may trigger additional agent inclusion for redundancy.
     *
