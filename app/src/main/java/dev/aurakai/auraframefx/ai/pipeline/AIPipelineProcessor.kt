...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Handles the complete lifecycle of an AI task: retrieves relevant context, determines task priority, selects appropriate agents, gathers their responses, synthesizes a final aggregated response, updates processing context and memory, and returns all agent messages generated during the process.
     *
    ...
    /**
     * Returns a list of recent task history entries, including previous task context and recent user interactions.
     *
    ...
    /**
     * Retrieves the current user preferences relevant to AI task processing.
     *
    ...
    /**
     * Calculates the priority of a given task based on its type, system load, and urgency indicators.
     *
     * The priority is determined by adjusting a base value according to the task's category, current system load, and the presence of urgency keywords in the task description. The result is clamped between 0.0 and 1.0.
     *
     ...
     * @return The calculated priority value between 0.0 (lowest) and 1.0 (highest).
     */
     ...
     *
     * Agents are chosen according to keywords in the task, the urgency indicated by priority, and task complexity.
     *
     ...
     *
     * Aggregates responses by agent type, highlights the primary Genesis analysis if present,
     * includes supplementary inputs from other agents with icons, and appends the average confidence score.
     *
     ...
     * @return The average confidence as a float in the range [0.0, 1.0].
     */
