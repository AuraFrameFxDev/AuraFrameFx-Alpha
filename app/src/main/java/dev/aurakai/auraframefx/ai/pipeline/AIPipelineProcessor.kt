...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Constructs a contextual map for a given task, including its category, recent history, user preferences, and system state.
     *
     * The returned map provides comprehensive context for the task, such as its type, timestamp, recent interactions, a descriptive string, user preferences, and current system metrics.
     *
     * @param task The task for which to build context.
     * @return A map containing contextual information relevant to the specified task.
     */
    ...
    /**
     * Retrieves a list of recent task contexts and user interactions for use in AI task processing.
     *
     * @return A list of strings summarizing recent task history and user activity.
     */
    ...
    /**
     * Retrieves the user's AI interaction preferences, such as response style and preferred agents.
     *
     * @return A map containing user preferences for AI responses.
     */
    ...
    /**
     * Retrieves the current system state, including load status, available agent count, and processing queue size.
     *
     * @return A map containing the keys "load", "available_agents", and "processing_queue" with their respective status values.
     */
    ...
    /**
     * Computes a task priority score based on task category, system load, and urgency keywords.
     *
     * Adjusts a base priority value according to the task's type, the current system load, and the presence of urgency indicators in the task description. The resulting score is clamped between 0.0 and 1.0.
     *
     * @param task The task description, used to detect urgency.
     * @param context Map containing contextual information such as task type and system state.
     ...
     *
     * Agents are selected by analyzing keywords in the task description, the computed priority score, and the task's length or word count. The Genesis agent is always included as the primary coordinator. Additional agents are added for tasks involving analysis, security, or creative generation, as well as for high-priority or complex tasks.
     *
     * @param task The task description to evaluate for agent selection.
     * @param priority The priority score of the task, used to determine agent redundancy.
    ...
    /**
     * Generates a structured, human-readable summary from multiple agent responses.
     *
     * Aggregates responses by agent type, highlights the Genesis agent's analysis, includes supplementary inputs from other agents with representative icons, and appends an overall confidence score.
     *
     * @param responses List of agent messages to be summarized.
     * @return A formatted string summarizing all agent responses and their combined confidence score.
     */
     ...
     *
     * @param responses List of agent messages containing confidence scores.
