...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Executes the complete lifecycle for the provided task: retrieves contextual information, determines task priority, selects relevant agents, collects their responses, synthesizes a final aggregated response, updates processing context, and returns all agent messages generated during processing.
     *
     ...
     *
     * The context map is used to provide relevant information for AI task processing and decision-making.
     *
     * @param task The task for which to generate contextual information.
     * @return A map containing the task, its category, timestamp, recent history, descriptive context, user preferences, and system state.
     */
    ...
    /**
     * Retrieves a list of recent task contexts and user interactions for contextual reference during AI processing.
     *
     * @return A list of strings representing recent tasks and user activities.
     */
    ...
    /**
     * Retrieves user preferences for AI responses, including response style and preferred agents.
     *
    ...
    /**
     * Retrieves the current operational state of the system, including load status, number of available agents, and processing queue size.
     *
     * @return A map with the keys "load" (system load level), "available_agents" (number of agents ready to process tasks), and "processing_queue" (number of tasks waiting to be processed).
     */
    ...
    /**
     * Computes a priority score for a task based on its category, system load, and urgency indicators.
     *
     * The score increases for tasks related to generation, analysis, assistance, or creation, decreases if the system load is high, and is boosted if the task contains urgency keywords such as "urgent", "asap", or "emergency". The final score is clamped between 0.0 and 1.0.
     *
     ...
     *
     * Genesis is always included. Cascade, Kai, and Aura agents are added if the task contains relevant keywords, if the priority is high (for redundancy), or if the task is complex (long or wordy).
     *
     * @param task The task description to analyze for agent selection.
     * @param priority The computed priority score, which can trigger additional agent inclusion for redundancy.
    ...
    /**
     * Synthesizes a structured summary from multiple agent messages, highlighting key analyses and confidence.
     *
     * The summary features the Genesis agent's core analysis, includes supplementary inputs from other agents with representative icons, and concludes with the overall average confidence score.
     *
     * @param responses The list of agent messages to aggregate.
     * @return A formatted summary string combining all agent responses and their collective confidence.
     */
