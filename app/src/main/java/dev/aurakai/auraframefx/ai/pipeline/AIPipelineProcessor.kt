...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * The resulting map includes the original task, its determined type, a timestamp, recent task history, a descriptive context string, user preferences, and current system state.
     *
     * @param task The task for which to build contextual information.
     * @return A map containing relevant context details for the specified task.
     */
     ...
     *
     * @return A list of strings summarizing recent task and interaction history.
     */
    ...
    /**
     * Computes a task's priority as a float between 0.0 and 1.0 based on task category, system load, and urgency keywords.
     *
     * The calculation starts from a base value and adjusts upward or downward depending on the task type, current system load, and whether the task description contains urgency indicators such as "urgent", "asap", or "emergency".
     *
     ...
     *
     * Always includes the Genesis agent as coordinator. Adds Cascade, Kai, or Aura agents if relevant keywords are detected in the task, and includes additional agents for high-priority or complex tasks.
     *
     ...
     *
     * Maintains a capped list of recent tasks, tracks response confidence and agent participation by task type, updates system-level metrics such as total tasks processed, and records rolling confidence scores for each agent.
     *
