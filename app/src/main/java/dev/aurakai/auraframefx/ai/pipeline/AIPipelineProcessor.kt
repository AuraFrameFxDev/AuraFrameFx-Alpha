...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Executes the complete task lifecycle: retrieves enriched context, calculates task priority, selects relevant agents, gathers and synthesizes agent responses, updates internal metrics, and returns all agent messages including the final aggregated result.
     *
     ...
     * @return A map containing the task details, categorized type, timestamp, recent task history, a contextual summary, user preferences, and system state.
     */
    ...
    /**
     * Categorizes a task description based on keyword matching.
     *
     * Returns "generation", "analysis", "explanation", "assistance", "creation", or "general" depending on the presence of specific keywords in the task description.
     *
    ...
    /**
     * Provides a static list of recent tasks or user interactions to enrich the processing context.
     *
    ...
    /**
     * Retrieves user preferences relevant to AI task processing, such as response style and preferred agents.
     *
     * @return A map containing user settings, including "response_style" and "preferred_agents".
     */
     ...
     *
     * The score is increased for tasks related to assistance, generation, or those containing urgency indicators, and decreased if the system load is high. The result is clamped between 0.0 and 1.0, where higher values indicate greater priority.
     *
     * @param task The task description to evaluate.
     * @param context Contextual information including task type and system state.
     * @return A float between 0.0 and 1.0 representing the computed priority of the task.
     */
    ...
    /**
     * Determines which AI agents should process a given task based on its content and priority.
     *
     * Genesis is always included. Cascade is added for analytical or data-related tasks, Kai for security-related tasks, and Aura for creative or generative tasks. High-priority or complex tasks result in additional agents being selected for broader coverage.
     *
    ...
    /**
     * Aggregates and formats multiple agent responses into a structured summary.
     *
    ...
    /**
     * Calculates the average confidence score from a list of agent messages, ensuring the result is within 0.0 to 1.0.
     *
