...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Processes an AI task by orchestrating multiple agents, aggregating their responses, and updating the pipeline state and context.
     *
     * Executes the complete task lifecycle: retrieves enriched context, calculates task priority, selects relevant agents, collects and aggregates agent responses, updates internal context and memory, and returns all generated agent messages including the final aggregated response.
     *
    ...
    /**
     * Categorizes a task description into a predefined type based on keyword detection.
     *
     ...
     *
     * @return A map with keys "load", "available_agents", and "processing_queue" describing the system's operational status.
     */
    ...
    /**
     * Calculates a normalized priority score for a task based on its type, system load, and urgency indicators.
     *
     * The score is increased for tasks related to assistance, generation, or containing urgency keywords, and decreased if the system load is high. The result is clamped between 0.0 and 1.0, where higher values indicate greater priority.
     *
     ...
     *
     * Genesis is always included. Cascade is added for analytical or data-related tasks, Kai for security-related tasks, and Aura for creative or generative tasks. High-priority (priority > 0.8) or complex tasks (length > 100 characters or more than 20 words) result in additional agents being included for redundancy and comprehensive coverage.
     *
