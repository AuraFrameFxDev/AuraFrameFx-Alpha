...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Constructs a context map for a given task, incorporating task categorization, recent history, user preferences, and system state.
     *
     * The resulting map includes the original task, its categorized type, a timestamp, recent task history, a descriptive context string, user preferences, and current system state.
     *
     ...
     * @return The category name: "generation", "analysis", "explanation", "assistance", "creation", or "general".
     */
    ...
    /**
     * Retrieves a static list of recent task context and user interactions for contextual reference in AI processing.
     *
    ...
    /**
     * Retrieves a map describing the current operational state of the AI system.
     *
     ...
     *
     * The priority is calculated by adjusting a base value according to the task type (e.g., generation, analysis, assistance), reducing it if the system load is high, and increasing it if urgency indicators are present in the task description. The result is clamped between 0.0 and 1.0.
     *
     ...
     *
     * The selection always includes the Genesis agent as coordinator. Additional agents are added if the task contains relevant keywords (e.g., analysis, security, creation), if the priority is high (for redundancy), or if the task is complex (long or wordy).
     *
    ...
    /**
     * Generates a structured summary of agent responses, highlighting primary and supplementary analyses with confidence metrics.
     *
     * Aggregates agent messages by type, prioritizes the Genesis agent's analysis, includes formatted inputs from other agents with representative icons, and appends the overall average confidence score.
     *
     * @param responses The list of agent messages to synthesize.
     * @return A formatted string summarizing all agent responses and their combined confidence, or a default message if no responses are available.
     */
