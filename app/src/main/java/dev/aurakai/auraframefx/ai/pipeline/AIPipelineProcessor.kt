...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Executes the full lifecycle for the provided task: retrieves enriched context, calculates task priority, selects relevant agents, gathers and synthesizes agent responses, updates internal context and metrics, and returns all agent messages including the final aggregated result.
     *
    ...
    /**
     * Constructs a detailed context map for the specified task, including its category, recent task history, user preferences, and current system state.
     *
     ...
     *
     * Returns one of: "generation", "analysis", "explanation", "assistance", "creation", or "general" if no relevant keywords are found.
     *
     ...
     *
     * The returned map includes the system load status, the number of available agents, and the current processing queue length.
     *
     ...
     *
     * Tasks related to assistance, generation, or containing urgency keywords receive higher scores, while high system load reduces priority. The resulting score is clamped between 0.0 and 1.0, with higher values indicating greater priority.
     *
    ...
    /**
     * Selects the set of AI agents best suited to process a task based on its content and priority.
     *
    ...
    /**
     * Aggregates and formats responses from multiple AI agents into a summary string.
     *
     * Highlights the Genesis agent's analysis as the primary response, appends supplementary responses from other agents with unique icons, and displays the overall average confidence score. Returns a default message if no agent responses are available.
     *
