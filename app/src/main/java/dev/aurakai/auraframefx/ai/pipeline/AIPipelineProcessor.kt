...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * The resulting map contains the original task, its categorized type, a timestamp, recent task history, a descriptive context string, user preferences, and the current system state. This context supports downstream AI processing and agent selection.
     *
    ...
    /**
     * Returns a list of recent task descriptions and user interactions to provide context for AI processing.
     *
     * @return A list of strings representing recent tasks and activities.
     */
     ...
     *
     * @return A map with keys such as "response_style" and "preferred_agents" representing user customization options.
     */
    ...
    /**
     * Returns a map containing the current system operational state, including load status, number of available agents, and processing queue size.
     *
     ...
     *
     * The score ranges from 0.0 (lowest priority) to 1.0 (highest priority), with adjustments based on task type, current system load, and the presence of urgency keywords in the task description.
     *
     ...
     *
     * Always includes the Genesis agent, and conditionally adds Cascade, Kai, and Aura agents depending on keywords, priority, and task length. High-priority or complex tasks may trigger additional agent inclusion for redundancy.
     *
    ...
    /**
     * Produces a formatted summary aggregating multiple agent messages into a human-readable response.
     *
     * Highlights the Genesis agent's analysis, presents supplementary inputs from other agents with icons, and appends the overall average confidence score.
     *
