...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Constructs a context map for a given task, including its category, recent history, user preferences, and system state.
     *
     * The context map provides comprehensive information to support AI task processing, such as the original task string, its categorized type, a timestamp, recent task history, a descriptive context summary, user preferences, and current system metrics.
     *
     ...
     * @return The category of the task: "generation", "analysis", "explanation", "assistance", "creation", or "general" if no keywords match.
     */
    ...
    /**
     * Retrieves a list of recent task contexts and user interactions for contextual AI processing.
     *
     ...
     *
     * The calculation increases priority for certain task types (e.g., assistance, generation), boosts or reduces it based on current system load, and further raises it if the task description contains urgency indicators such as "urgent", "asap", or "emergency".
     *
     ...
     *
     * The Genesis agent is always included. Additional agents (Cascade, Kai, Aura) are selected if the task contains relevant keywords, if the priority is high (for redundancy), or if the task is complex.
     *
     * @param task The description of the task to analyze.
     * @param priority The computed priority of the task, affecting agent selection for redundancy.
    ...
    /**
     * Generates a structured summary of agent responses, highlighting the Genesis agent's analysis and including supplementary inputs from other agents.
     *
     * Aggregates responses by agent type, formats each with appropriate icons and headers, and appends an overall confidence score. Returns a formatted string suitable for display.
     *
