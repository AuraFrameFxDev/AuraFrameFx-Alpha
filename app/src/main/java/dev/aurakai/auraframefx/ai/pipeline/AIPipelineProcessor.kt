...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Coordinates the processing of an AI task by engaging multiple agents, aggregating their responses, and updating pipeline state and context.
     *
     * Executes the complete task lifecycle: retrieves contextual information, determines task priority, selects relevant agents, collects their responses, synthesizes a final aggregated response, updates processing context, and returns all agent messages generated during processing.
     *
     ...
     *
     * The returned map provides the original task, its determined category, a timestamp, recent task history, a descriptive context string, user preferences, and the current system state.
     *
     * @param task The task for which contextual information is generated.
     * @return A map containing detailed context relevant to the specified task.
     */
    ...
    /**
     * Returns a category label for the given task description based on the presence of specific keywords.
     *
    ...
    /**
     * Retrieves a static list of recent task context and user interaction descriptions for contextual reference in AI processing.
     *
    ...
    /**
     * Retrieves a map of the user's AI interaction preferences, such as response style and preferred agents.
     *
     ...
     *
     * The score is adjusted upward or downward depending on the task type, the system's load status, and whether the task description contains urgency indicators such as "urgent", "asap", or "emergency".
     *
     ...
     *
     * Agents are selected by analyzing keywords in the task description, the computed priority score, and the task's length or word count. The GENESIS agent is always included as the primary coordinator. Additional agents are added for tasks requiring analysis, security, creative generation, high urgency, or increased complexity.
     *
    ...
    /**
     * Creates a formatted summary of multiple agent responses, highlighting primary and supplementary analyses.
     *
     * Aggregates agent messages by type, prioritizes the Genesis agent's analysis, includes additional inputs from other agents with representative icons, and appends the overall average confidence score.
     *
     * @param responses The list of agent messages to summarize.
     * @return A human-readable string combining all agent responses and their aggregated confidence.
     */
    ...
    /**
     * Computes the average confidence score from the provided agent messages, ensuring the result is within [0.0, 1.0].
     *
