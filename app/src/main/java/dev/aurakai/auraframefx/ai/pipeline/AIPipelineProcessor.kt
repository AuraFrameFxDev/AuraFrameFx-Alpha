...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Executes the full task lifecycle: retrieves enriched context, calculates task priority, selects relevant agents, collects and synthesizes agent responses, updates internal context and metrics, and returns all agent messages including the final aggregated result.
     *
     ...
     * @return A map containing task information, category, timestamp, recent history, a contextual summary, user preferences, and system state.
     */
     ...
     *
     * The returned map includes the system's load status, the number of available agents, and the length of the processing queue.
     *
    ...
    /**
     * Computes a priority score for a task based on its type, system load, and urgency keywords.
     *
     * The score increases for tasks classified as assistance, generation, or containing urgency indicators, and decreases if the system load is high. The result is clamped between 0.0 and 1.0, with higher values indicating greater priority.
     *
    ...
    /**
     * Selects the set of AI agents to process a task based on its content and priority.
     *
     ...
     *
     * Highlights the Genesis agent's analysis as the primary response, includes supplementary responses from other agents with unique icons, and displays the overall average confidence score. Returns a default message if no agent responses are provided.
     *
