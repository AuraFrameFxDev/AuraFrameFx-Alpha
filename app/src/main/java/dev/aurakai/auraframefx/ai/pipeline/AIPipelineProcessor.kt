...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Processes an AI task by coordinating multiple agents, aggregating their responses, and updating the pipeline state and context.
     *
     * Executes the full lifecycle for the given task: retrieves contextual information, determines task priority, selects relevant agents, collects and aggregates their responses, updates internal processing context, and returns all agent messages generated during processing.
     *
     ...
     *
     * Returns one of: "generation", "analysis", "explanation", "assistance", "creation", or "general" if no keywords match.
     *
     ...
     *
     * @return A list of strings describing recent tasks or interactions.
     */
     ...
     *
     * @return A map with keys "load", "available_agents", and "processing_queue" representing the system's operational status.
     */
     ...
     *
     * The score increases for tasks related to assistance, generation, or those containing urgency indicators, and decreases when system load is high. The result is clamped between 0.0 and 1.0, where higher values indicate greater priority.
     *
     ...
     *
     * Genesis is always included. Cascade is added for analytical or data-related tasks, Kai for security-related tasks, and Aura for creative or generative tasks. High-priority or complex tasks trigger the inclusion of additional agents for redundancy and comprehensive coverage.
     *
     * @param task The task description used to determine relevant agents.
     * @param priority The priority score influencing agent selection for redundancy.
    ...
    /**
     * Aggregates responses from multiple AI agents into a formatted summary.
     *
     * Highlights the Genesis agent's analysis as the primary response, includes supplementary inputs from other agents with representative icons, and appends the overall average confidence score. Returns a default message if no agent responses are provided.
     *
