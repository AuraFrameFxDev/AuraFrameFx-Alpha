...
class AIPipelineProcessor @Inject constructor(
    ...
    /**
     * Executes the complete AI task processing lifecycle by coordinating multiple specialized agents, synthesizing their responses, and updating pipeline state and context.
     *
     * Retrieves enriched context for the given task, determines its priority, selects appropriate agents, collects and aggregates their responses, updates internal metrics and context, and returns all agent messages including the final synthesized result.
     *
     ...
     * @return A map containing the task, its categorized type, timestamp, recent task history, a contextual summary, user preferences, and system state.
     */
     ...
     *
     * Returns "generation", "analysis", "explanation", "assistance", "creation", or "general" depending on which keyword is found in the task description.
     *
    ...
    /**
     * Returns a simulated snapshot of the current system state, including load status, available agent count, and processing queue length.
     *
    ...
    /**
     * Calculates a priority score for the given task using its type, system load, and urgency indicators.
     *
     * The score is increased for tasks related to assistance, generation, or those containing urgency keywords, and decreased if the system load is high. The result is clamped between 0.0 and 1.0, where higher values indicate greater priority.
     *
     * @param task The task description to evaluate.
     ...
     *
     * Genesis is always selected. Cascade is added for analytical or data-related tasks, Kai for security-related tasks, and Aura for creative or generative tasks. Tasks with high priority or greater complexity result in additional agents being included for broader coverage.
     *
    ...
    /**
     * Aggregates and formats responses from multiple AI agents into a comprehensive summary.
     *
