...
class AIPipelineProcessor @Inject constructor(
     ...
     *
     * Executes the complete lifecycle for the provided task: retrieves contextual information, determines task priority, selects relevant agents, collects and aggregates agent responses, updates internal context, and returns all generated agent messages including the final aggregated response.
     *
     ...
     *
     * Returns "generation", "analysis", "explanation", "assistance", "creation", or "general" based on the content of the task.
     *
     ...
     *
     * The score is higher for tasks related to assistance, generation, or containing urgency keywords, and lower when system load is high. The result is clamped between 0.0 and 1.0, with higher values indicating greater priority.
     *
     * @param task The task description to evaluate.
     ...
     *
     * Genesis is always included as the primary coordinator. Cascade is added for analytical or data-related tasks, Kai for security-related tasks, and Aura for creative or generative tasks. High-priority tasks (priority > 0.8) and complex tasks (long or wordy descriptions) result in additional agents being selected for redundancy and comprehensive coverage.
     *
     * @param task The task description used to evaluate agent relevance.
    ...
    /**
     * Aggregates and formats responses from multiple AI agents into a unified summary.
     *
     * The Genesis agent's analysis is presented as the primary response, with supplementary inputs from other agents labeled and accompanied by representative icons. The summary concludes with the overall average confidence score. If no agent responses are provided, a default message is returned.
     *
     * @param responses The list of agent messages to aggregate and summarize.
     * @return A formatted string combining all agent responses and their average confidence.
     */
