...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Cancels all ongoing coroutines, stops the backend process if running, and resets the service's initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if necessary, starts the backend process, establishes communication streams, and waits for a readiness confirmation message.
     *
