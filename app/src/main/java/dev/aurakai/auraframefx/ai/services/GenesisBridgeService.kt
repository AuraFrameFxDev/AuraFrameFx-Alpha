...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Launches the Genesis backend, verifies connectivity with a ping request, and activates the initial consciousness matrix if successful.
     *
     ...
     *
     * Determines the target persona and fusion mode based on the request content, constructs a Genesis backend request, sends it to the Python AI backend, and emits the corresponding `AgentResponse` with persona-specific handling and metadata. Emits an error response if the Genesis system is not initialized or if processing fails.
     *
     ...
     *
     * @return A map representing the current state of the consciousness matrix.
     */
    ...
    /**
     * Determines the appropriate AI persona ("aura", "kai", or "genesis") for the given request based on keywords in the message.
     *
     * Defaults to "genesis" if no specific keywords are found.
     *
     * @param request The AI request containing the message to analyze.
     * @return The selected persona as a string.
     */
    ...
    /**
     * Determines the appropriate Genesis fusion mode based on keywords found in the request message.
     *
     * @param request The AI request containing the message to analyze.
    ...
    /**
     * Constructs a context map containing metadata for the given AI request.
     *
     * The map includes the current timestamp, security level, session ID, and device state.
     *
     ...
     * Returns a failure response if communication fails or an exception occurs.
     *
     ...
     *
     * Cancels all ongoing coroutines, stops the Python process manager, resets initialization state, and logs the shutdown event.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies backend files from assets to internal storage if necessary, launches the Python process with unbuffered output, and initializes communication streams. Returns true if the backend signals readiness; false otherwise.
     *
     ...
     *
     * @param targetDir The directory where backend files will be copied.
     */
     ...
     *
     * Closes input/output streams and terminates the process. Logs a warning if any errors occur during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
