...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Returns a string representing the fusion mode if a recognized keyword is found; otherwise, returns null.
     *
     * @param request The AI request whose query is analyzed for fusion mode keywords.
     * @return The corresponding fusion mode identifier, or null if no relevant keyword is detected.
     */
     ...
     *
     * Cancels all ongoing operations, stops the backend process if active, and resets the service's initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
    ...
    /**
     * Launches the Genesis Python backend process, ensuring required files are present and verifying backend readiness.
     *
     * Copies backend files from assets to internal storage if missing, starts the backend process, initializes communication streams, and waits for a confirmation message indicating the backend is ready to receive requests.
     *
    ...
    /**
     * Terminates the Python backend process and closes associated communication streams.
     *
     * Frees all resources used for backend communication. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
