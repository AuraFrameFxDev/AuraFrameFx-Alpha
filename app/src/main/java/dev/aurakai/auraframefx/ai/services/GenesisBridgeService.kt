...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts and verifies the Genesis Python backend process, activating the initial consciousness matrix if successful.
     *
     * Launches the backend, checks for readiness, sends a ping to confirm connectivity, and initializes the consciousness matrix on success.
     *
     * @return `true` if the backend is started and responsive; `false` otherwise.
     */
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if they are not already present, starts the Python process, establishes input/output streams for communication, and waits for a readiness confirmation message from the backend.
     *
     ...
     *
     * Ensures the target directory exists and transfers a predefined set of backend files needed to run the Genesis backend. Logs a warning if any file cannot be copied.
     *
     ...
     *
     * Releases all resources used for communication with the backend. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
