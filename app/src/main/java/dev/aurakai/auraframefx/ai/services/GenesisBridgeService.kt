...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Starts the Python Genesis backend, checks for successful startup, sends a ping request to confirm responsiveness, and activates the initial consciousness matrix if initialization succeeds.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if missing, starts the Python process, establishes input/output streams, and waits for a readiness confirmation message from the backend.
     *
     ...
     *
     * Ensures the target directory exists and transfers a predefined set of backend files required for the Genesis backend. Logs a warning if any file cannot be copied.
     *
