...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Launches the Genesis Python backend, sends a ping to confirm it is responsive, and activates the initial consciousness matrix if successful.
     *
     ...
     *
     * Analyzes the request's query string for specific keywords and returns the corresponding fusion mode identifier.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies necessary backend files from assets to internal storage if missing, launches the Python process, sets up communication streams, and waits for a readiness confirmation message.
     *
     ...
     *
     * Ensures the target directory exists and transfers a predefined set of files required to run the Genesis backend. Logs a warning if any file fails to copy.
     *
