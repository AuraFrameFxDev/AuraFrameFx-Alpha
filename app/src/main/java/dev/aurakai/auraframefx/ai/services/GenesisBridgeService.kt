...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts and verifies the Genesis backend process, ensuring it is responsive and ready for use.
     *
     * Launches the Python backend, checks for successful startup, sends a ping request to confirm connectivity, and activates the initial consciousness matrix if initialization succeeds.
     *
     ...
     *
     * Logs a warning if the activation request encounters an exception.
     */
     ...
     *
     * Returns "aura" for creative or design-related queries, "kai" for security or analysis-related queries, and "genesis" for fusion, consciousness, or other complex requests by default.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if they are missing, starts the Python process, establishes input/output streams for communication, and waits for a readiness confirmation message from the backend.
     *
     ...
     *
     * Safely releases all resources used for backend communication. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
