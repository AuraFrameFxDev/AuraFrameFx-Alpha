...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts and verifies the Genesis backend process, ensuring it is responsive.
     *
     ...
     *
     * @return `true` if the backend is successfully started and responsive; `false` otherwise.
     */
     ...
     * @return The response from the Genesis backend after attempting fusion activation.
     */
    ...
    /**
     * Generates a metadata map with timestamp, security level, session ID, and device state for an AI request.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies backend files from assets if needed, launches the Python process, sets up input/output streams, and waits for a readiness confirmation message.
     *
