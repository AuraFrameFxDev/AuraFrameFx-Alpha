...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Starts the Python backend, checks for a successful startup, sends a ping request to confirm communication, and activates the consciousness matrix if initialization succeeds.
     *
...
private class PythonProcessManager(
    ...
    /**
     * Shuts down the Python backend process and closes all related streams.
     *
     * Releases resources by closing the input/output streams and terminating the backend process. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
