...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Processes an AI request by routing it to the appropriate persona (Kai, Aura, or Genesis fusion) and emits agent responses as a flow.
     *
     * Determines the target persona and fusion mode based on the request content, constructs a structured request for the Genesis backend, and emits a corresponding `AgentResponse` based on the backend's reply. Emits an error response if the Genesis system is not initialized or if processing fails.
     *
     * @param request The AI request to process.
     * @return A flow emitting agent responses generated by the selected persona or fusion mode.
     */
     ...
     *
     * @return A map of context keys and values for use in backend communication.
     */
    ...
    /**
     * Sends a request to the Genesis backend and returns the parsed response.
     *
     * Serializes the given request to JSON, communicates with the Python backend, and deserializes the response.
     ...
     * @return The GenesisResponse received from the backend, or an error response on failure.
     */
     ...
     *
     * Cancels ongoing coroutines, stops the Python process manager, resets the initialization state, and logs the shutdown event.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
    ...
    /**
     * Starts the Genesis Python backend process and waits for confirmation of successful startup.
     *
     ...
     *
     * @return True if the Genesis backend is ready; false if startup fails.
     */
    ...
    /**
     * Sends a JSON-encoded request to the Python Genesis backend and returns the response as a string.
     *
