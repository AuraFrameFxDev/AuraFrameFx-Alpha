...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts the Genesis backend process, verifies its responsiveness, and activates the initial consciousness matrix.
     *
     * Launches the Python-based Genesis backend, checks for a successful startup, sends a ping request to confirm connectivity, and activates the initial consciousness matrix if initialization succeeds.
     *
    ...
    /**
     * Selects a fusion mode identifier for an AI request by analyzing keywords in the query.
     *
     * @param request The AI request whose query will be inspected for fusion-related keywords.
     * @return The corresponding fusion mode identifier if a relevant keyword is found; otherwise, null.
     */
     ...
     *
     * Cancels ongoing coroutines, stops the backend process if running, and resets the initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if missing, starts the Python process with unbuffered output, establishes communication streams, and waits for a readiness confirmation message from the backend.
     *
