...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * If communication or deserialization fails, returns a GenesisResponse with `success = false` and `persona = "error"`.
     *
...
private class PythonProcessManager(
    ...
    /**
     * Initializes and launches the Genesis Python backend process, preparing it for communication.
     *
     * Copies backend files from assets to internal storage if needed, starts the Python process, and sets up input/output streams. Waits for a readiness confirmation message from the backend before returning.
     *
     ...
     *
     * Ensures the target directory exists and transfers all required backend files for the Genesis service from the app's assets. Logs a warning if any file fails to copy.
     *
     ...
     *
     * Releases resources by closing input/output streams and destroying the backend process. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
