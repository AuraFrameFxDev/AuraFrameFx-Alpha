...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts the Genesis backend process, verifies connectivity, and activates the initial consciousness matrix.
     *
     ...
     *
     * Sends a request to the backend to update its internal state with relevant Android and app metadata.
     ...
     *
     * Returns "aura" for queries related to creativity or design, "kai" for security or analysis, and "genesis" for fusion, consciousness, or by default for other cases.
     *
    ...
    /**
     * Returns the fusion mode identifier corresponding to specific keywords found in the AI request query.
     *
     ...
     *
     * Cancels all ongoing background operations, stops the Python backend if running, and resets the service's initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if they are missing, starts the Python process, establishes communication streams, and waits for a readiness confirmation message from the backend.
     *
