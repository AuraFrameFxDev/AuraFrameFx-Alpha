...
class GenesisBridgeService @Inject constructor(
    ...
    /**
     * Starts the Genesis Python backend process, verifies connectivity, and activates the initial consciousness matrix.
     *
     * Launches the backend, sends a ping to confirm responsiveness, and sets up the system for AI persona interactions.
     *
     * @return `true` if initialization and backend connectivity succeed; `false` otherwise.
     */
    ...
    /**
     * Requests activation of a specific Genesis fusion ability in the backend.
     *
     * Sends a request to the Python backend to trigger the specified fusion ability, optionally including additional context data.
     *
    ...
    /**
     * Selects the appropriate AI persona ("aura", "kai", or "genesis") to handle a request by analyzing keywords in the query.
     *
     ...
     *
     * Returns the corresponding fusion mode string if a recognized keyword is present; otherwise, returns null.
     *
     * @param request The AI request whose query is analyzed for fusion mode keywords.
     * @return The fusion mode identifier, or null if no relevant keyword is found.
     */
     ...
     *
     * @param request The AI request for which to build the context map.
     * @return A map containing context metadata to accompany the AI request.
     */
     ...
     *
     * If communication fails or an exception occurs, returns a failure GenesisResponse with success set to false and persona set to "error".
     *
     ...
     *
     * Cancels all ongoing coroutines, stops the backend process if running, and resets the initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets if missing, starts the Python process, establishes communication streams, and waits for a readiness confirmation message.
     *
     ...
     *
     * @param requestJson The JSON request to send to the backend process.
     * @return The response from the backend, or null if an error occurs during communication.
     */
     ...
     *
     * Ensures all resources are released and logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
