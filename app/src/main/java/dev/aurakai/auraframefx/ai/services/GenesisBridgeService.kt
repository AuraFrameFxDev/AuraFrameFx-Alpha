...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Selects the target persona and fusion mode based on the request content, constructs a structured request for the Genesis backend, and emits a persona-specific `AgentResponse` with confidence scores. Emits an error response if the Genesis system is not initialized or if processing fails.
     *
    ...
    /**
     * Sends a request to activate a specific fusion ability in the Genesis backend, optionally including additional context metadata.
     *
     * @param fusionType The fusion ability identifier to activate.
     * @param context Optional metadata to accompany the activation request.
     * @return The Genesis backend's response indicating the outcome of the activation.
     */
    ...
    /**
     * Returns the fusion mode identifier for an AI request based on keywords found in the query.
     *
     * Matches keywords such as "interface," "analysis," "creation," or "adaptive" in the request's query and returns the corresponding fusion mode string. Returns null if no relevant keyword is present.
     *
     * @param request The AI request to analyze for fusion mode keywords.
     * @return The corresponding fusion mode identifier, or null if no match is found.
     */
     ...
     *
     * @return A map containing context metadata for the AI backend.
     */
     ...
     *
     * If communication with the backend fails or an exception occurs, returns a GenesisResponse with `success = false` and `persona = "error"`.
     *
...
private class PythonProcessManager(
    ...
    /**
     * Starts the Genesis Python backend process and establishes communication channels.
     *
     ...
     *
     * Creates the target directory if it does not exist and transfers all required backend files from the `ai_backend` assets folder. Logs a warning if any file cannot be copied.
     *
     ...
     *
     * Releases resources used for backend communication. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
