...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Returns "aura" for creative or design-related queries, "kai" for security or analysis-related queries, and "genesis" for fusion, consciousness, or other complex or default cases.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies backend files from assets if needed, launches the Python process, sets up input/output streams, and waits for a readiness confirmation message from the backend.
     *
     * @return `true` if the backend process is successfully started and confirms readiness; `false` otherwise.
     */
     ...
     *
     * Releases all resources associated with the backend process, including input and output streams. Logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
