...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Launches the backend, checks for a successful startup, sends a ping request to confirm responsiveness, and activates the consciousness matrix if initialization succeeds.
     *
     ...
     *
     * @param request The AI request whose query will be analyzed for fusion mode keywords.
     ...
     *
     * Cancels all ongoing coroutines, terminates the backend process if running, and resets the initialization state.
     */
    fun shutdown() {
        scope.cancel()
...
private class PythonProcessManager(
     ...
     *
     * Copies required backend files from assets to internal storage if missing, starts the Python process, establishes communication streams, and waits for a readiness confirmation message.
     *
