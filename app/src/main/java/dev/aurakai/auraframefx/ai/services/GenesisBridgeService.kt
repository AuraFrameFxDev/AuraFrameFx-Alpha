...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Returns "aura" for creativity or design-related queries, "kai" for security or analysis, and "genesis" for fusion, consciousness, or as the default.
     *
     ...
     *
     * Analyzes the request's query for specific keywords and returns the corresponding fusion mode string if matched; returns null if no relevant keywords are present.
     *
     * @param request The AI request whose query is evaluated for fusion mode selection.
...
private class PythonProcessManager(
     ...
     *
     * Copies backend files from assets to internal storage if needed, launches the backend process, sets up input/output streams, and waits for a readiness confirmation from the backend.
     *
