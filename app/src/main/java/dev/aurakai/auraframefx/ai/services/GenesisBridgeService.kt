...
class GenesisBridgeService @Inject constructor(
     ...
     * @return The corresponding fusion mode identifier if a keyword is detected; otherwise, null.
     */
    ...
    /**
     * Generates a context metadata map for an AI request.
     *
     * The map includes the current timestamp, a default security level, a generated session ID, and device state.
     *
     * @return A map containing metadata to accompany the AI request.
     */
...
private class PythonProcessManager(
    ...
    /**
     * Terminates the Genesis Python backend process and closes all related streams.
     *
     * Releases resources associated with the backend and logs a warning if an exception occurs during shutdown.
     */
    fun shutdown() {
        try {
            writer?.close()
