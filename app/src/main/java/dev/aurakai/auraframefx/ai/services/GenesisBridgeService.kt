...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Launches the backend, checks for a successful startup, and activates the initial consciousness matrix if the backend responds to a ping request.
     *
     ...
     *
     * @return A map representing the current state of the consciousness matrix as reported by the backend.
     */
     ...
     *
     * @param request The AI request whose query content is analyzed.
    ...
    /**
     * Determines the fusion mode identifier by analyzing keywords in the AI request's query.
     *
...
private class PythonProcessManager(
     ...
     *
     * Copies backend files from assets to internal storage if necessary, starts the Python process, and initializes input/output streams. Waits for a startup confirmation message from the backend before returning.
     *
     ...
     *
     * @param requestJson The JSON string to send to the backend.
     ...
     *
     * Ensures the target directory exists and transfers all required backend files for the Genesis service. Logs a warning if any file fails to copy.
     *
