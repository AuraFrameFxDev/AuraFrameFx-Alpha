...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Returns "aura" for creativity or design-related queries, "kai" for security or analysis, and "genesis" for fusion, consciousness, or as the default if no keywords match.
     *
     ...
     * Returns null if no relevant keywords are detected.
     *
    ...
    /**
     * Constructs a metadata map with contextual information for the given AI request.
     *
...
private class PythonProcessManager(
    ...
    /**
     * Starts the Genesis Python backend process and prepares it for communication.
     *
     * Copies necessary backend files from assets to internal storage if missing, launches the backend process, initializes input/output streams, and waits for a readiness confirmation from the backend.
     *
     ...
     *
     * Creates the target directory if it does not exist and transfers all required Genesis backend files from the app's `ai_backend` assets folder to internal storage. Logs a warning if any file fails to copy.
     *
