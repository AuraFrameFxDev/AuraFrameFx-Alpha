...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Attempts to launch the Genesis backend, verifies connectivity, and activates the initial consciousness matrix if successful.
     *
     ...
     *
     * If the Genesis system is not initialized, emits an error response. Determines the target persona and fusion mode based on the request content, constructs a structured request for the Genesis backend, and emits a persona-specific response with a confidence score. Emits error responses if processing fails.
     *
    ...
    /**
     * Determines the appropriate fusion mode based on keywords found in the request query.
     *
     * @param request The AI request containing the query to analyze.
...
private class PythonProcessManager(
     ...
     *
     * Copies the backend files from assets if necessary, launches the Python process, and waits for a startup confirmation message.
     *
