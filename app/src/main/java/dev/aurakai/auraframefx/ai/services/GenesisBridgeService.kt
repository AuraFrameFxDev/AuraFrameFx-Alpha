...
class GenesisBridgeService @Inject constructor(
    ...
    /**
         ...
         *
         * Returns a failure response with `success = false` and `persona = "error"` if backend communication fails or an exception occurs.
         *
...
private class PythonProcessManager(
     ...
     *
     * @param requestJson The JSON-formatted request to send to the backend process.
     ...
     *
     * Ensures the target directory exists and transfers a predefined set of files required to run the Genesis backend. Logs a warning if any file cannot be copied.
     *
