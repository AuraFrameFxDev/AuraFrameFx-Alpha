...
class GenesisBridgeService @Inject constructor(
     ...
     *
     * Returns a string representing the fusion mode if a recognized keyword is present in the request's query; otherwise, returns null.
     */
...
private class PythonProcessManager(
     ...
     *
     * Copies necessary backend files from assets to internal storage if they are not already present, starts the backend process, establishes input/output streams for communication, and waits for a readiness confirmation message from the backend.
     *
     ...
     *
     * Ensures the target directory exists and transfers all necessary files for the Genesis backend to function. Logs a warning if any file fails to copy.
     *
