...
class TaskScheduler @Inject constructor(
     ...
     *
     * Returns `true` if all dependencies are completed and required agents are specified (agent availability check is not yet implemented); otherwise, returns `false`.
     */
    ...
    /**
     * Marks the given task as in progress, assigns required agents, and moves it to the active tasks list.
     *
     * Updates the task's status and agent assignment, then reflects these changes in the active and overall task collections.
     */
     ...
     *
     * If the status is `COMPLETED`, moves the task from active to completed. If the status is `FAILED`, removes it from active tasks and reports the failure to the error handler. Updates task statistics and triggers further scheduling.
     *
    ...
    /**
     * Calculates the importance score for a task using its importance value and the configured importance weight.
     *
