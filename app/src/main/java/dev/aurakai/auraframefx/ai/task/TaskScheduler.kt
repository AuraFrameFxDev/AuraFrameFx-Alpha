...
class TaskScheduler @Inject constructor(
    ...
    /**
     * Determines whether the specified task is eligible for execution based on its dependencies and required agents.
     *
     * Returns `true` if all dependencies are completed and agent requirements are met; otherwise, returns `false`.
     */
     ...
     *
     * Updates the task's status to `IN_PROGRESS`, assigns its required agents, adds it to the active tasks map,
     * and updates the tracked tasks state.
     *
     ...
     * If the status is `FAILED`, removes the task from active tasks and reports the failure to the error handler.
     * Updates the task in the task map, refreshes task statistics, and triggers processing of the task queue.
     *
