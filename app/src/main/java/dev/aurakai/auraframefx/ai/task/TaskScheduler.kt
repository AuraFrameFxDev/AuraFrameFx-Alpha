...
class TaskScheduler @Inject constructor(
    ...
    /**
     * Marks the given task as in progress, assigns required agents, and adds it to the set of active tasks.
     *
     * Updates the internal task map to reflect the task's new status and assignment.
     */
     ...
     *
     * If the status is `COMPLETED`, moves the task from active to completed. If the status is `FAILED`, removes it from active tasks and reports the failure via the error handler. Updates the task record, statistics, and triggers further queue processing.
     *
