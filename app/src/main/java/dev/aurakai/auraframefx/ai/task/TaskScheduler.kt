...
class TaskScheduler @Inject constructor(
     ...
     *
     * Returns `true` if all dependencies are completed and agent requirements (if any) are considered available.
     * Currently, agent availability checks are not implemented and always return `true` if agents are required.
     *
    ...
    /**
     * Marks the given task as in progress, assigns required agents, and moves it to the active tasks map.
     *
     * Updates the global task map to reflect the task's new status and assignment.
     */
     ...
     *
     * If the status is `COMPLETED`, moves the task from active to completed tasks. If the status is `FAILED`, removes it from active tasks and reports the failure to the error handler. Updates the global task map, task statistics, and triggers further queue processing.
     *
     ...
     *
     * @return The weighted urgency score for the given task.
     */
     ...
     * records the last update timestamp, and adjusts the count for the task's current status.
     *
