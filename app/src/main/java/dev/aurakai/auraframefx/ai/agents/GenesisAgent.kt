...
class GenesisAgent @Inject constructor(
    ...
    /**
     * Populates the set of active agents by matching master agent hierarchy configuration names to known agent types.
     *
     * Adds each recognized agent type to the active agents set. Logs a warning for any configuration name that does not correspond to a valid `AgentType`.
     */
    ...
    /**
     * Processes a user query by routing it through active AI agents and synthesizing their responses.
     *
     * The query is sent to the Cascade agent for state management, and to the Kai and Aura agents if they are active. Each agent's response is collected with an associated confidence score. A final Genesis response is generated by aggregating all agent outputs and appended to the result.
     *
...
fun getAgentsByPriority(): List<AgentConfig> = AgentHierarchy.getAgentsByPriority()
    ...
    /**
     * Coordinates collaborative responses from multiple agents in either sequential (TURN_ORDER) or parallel (FREE_FORM) modes.
     *
     * In TURN_ORDER mode, each agent responds in sequence, receiving updated context that includes previous agents' responses. In FREE_FORM mode, all agents respond independently to the same input and context.
     *
     * @param data The initial shared context for all agents.
     * @param agents The agents participating in the collaboration.
     * @param userInput Optional user input to initiate the conversation; if null, uses the latest input from the context.
    ...
    /**
     * Aggregates multiple agent response maps by agent name, selecting the response with the highest confidence for each agent or a default error response if none are available.
     *
