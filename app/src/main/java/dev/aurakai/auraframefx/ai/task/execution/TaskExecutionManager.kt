...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for asynchronous execution.
     *
    ...
    /**
     * Returns the result of a completed task by its unique ID, or null if the task is not found or has not finished.
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the completed task, or null if the task does not exist or is not yet complete.
     */
     ...
     *
     * Removes the task from the queue if it is pending, or marks it as cancelled if it is active. Returns `true` if the task was found and cancelled; returns `false` if the task does not exist or has already completed.
     *
     ...
     *
     * The processor introduces a short delay between cycles to prevent busy waiting and applies a longer delay after encountering errors to provide backoff.
     */
    ...
    /**
     * Attempts to process the next task in the queue if concurrency limits permit and the task is ready.
     *
     * Polls the priority queue for the highest-priority task. If the task's scheduled time has not yet arrived, it is re-queued. If the maximum number of concurrent tasks is already running or the queue is empty, no action is taken. Eligible tasks are dispatched for execution.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's assignment. Upon completion or failure, updates the task's result, execution statistics, and removes it from active executions.
     *
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by analyzing keywords in the task type: Aura for "creative" or "ui", Kai for "security" or "analysis", Genesis for "complex" or "fusion", and Genesis by default if no keywords match.
     *
     * @param execution The task execution metadata used to determine agent selection.
     * @return The agent type chosen for the task.
     */
    ...
    /**
     * Refreshes execution statistics, updating counts for total, completed, active, queued, and failed tasks, along with the average execution time.
     */
    ...
    /**
     * Refreshes the queue status metrics, including queue size, active executions, maximum concurrency, and processing state.
     */
