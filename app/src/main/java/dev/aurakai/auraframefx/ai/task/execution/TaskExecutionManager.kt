...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Searches active, completed, and queued tasks for the specified task ID and returns its status, or `null` if the task is not found.
     *
     ...
     *
     * Removes the task from the queue or marks an active task as cancelled, signaling its execution to terminate.
     *
     * @param taskId The unique identifier of the task to cancel.
     * @return `true` if the task was successfully cancelled; `false` if the task was not found or already completed.
     */
     ...
     *
     * The processor executes pending tasks, applies a short delay between iterations to avoid busy waiting, and uses a longer delay after errors to provide backoff. Processing continues until explicitly stopped.
     */
     ...
     *
     * Polls the priority queue for the next eligible task, checks if its scheduled time has arrived, and initiates execution if ready. If the task is scheduled for a future time, it is re-queued.
     */
     ...
     *
     * Constructs an `AiRequest` from the task's type and input data, then delegates processing to the Aura agent.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task's type, data, and priority, then processes it with the Kai agent.
     *
     * @param execution The task execution containing type, data, and priority.
     ...
     *
     * Constructs an `AgentRequest` from the task's data and metadata, then delegates processing to the Genesis agent.
     *
     * @param execution The task execution containing type, data, and priority.
    ...
    /**
     * Refreshes the execution statistics state flow with up-to-date counts of total, completed, active, queued, and failed tasks, and recalculates the average execution time.
     */
