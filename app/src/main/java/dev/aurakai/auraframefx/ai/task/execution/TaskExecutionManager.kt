...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution`, determines the optimal agent if not specified, and enqueues the task for processing.
     *
     ...
     * @return The execution status if found, or `null` if the task does not exist.
     */
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the task if it has completed, or `null` if the task is not found or is not yet completed.
     */
     ...
     *
     * Removes the task from the queue if it has not started, or marks an active task as cancelled to signal interruption.
     *
     ...
     *
     * @param status If specified, only tasks with this execution status are returned.
     ...
     *
     * Introduces a short delay between processing cycles to reduce resource usage, and applies a longer delay after exceptions to provide error backoff.
     */
     ...
     *
     * Polls the priority queue for a task whose scheduled time has arrived. If the concurrency limit is reached, no task is available, or the next task is scheduled for the future, the function returns without processing. Tasks not yet ready are re-queued for later execution.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's assignment. Upon completion or failure, stores the result, updates execution statistics, and removes the task from active executions.
     *
     ...
     *
     * Converts the task's type and data into an `AiRequest` and delegates it to the Aura agent for processing.
     *
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, then delegates processing to the Kai agent.
     *
     * @param execution The task execution containing details for the request.
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, then delegates processing to the Genesis agent.
     *
     * @return The response from the Genesis agent for the executed task.
     */
    ...
    /**
     * Determines the most suitable agent type for a task based on agent preference or task type keywords.
     *
     * Uses the specified agent preference if valid; otherwise, analyzes the task type string for keywords to select an agent. Defaults to Genesis if no preference or keyword match is found.
     *
     * @param execution The task execution metadata containing agent preference and task type.
     * @return The agent type selected for executing the task.
     */
    ...
    /**
     * Updates the queue status state flow with the latest queue size, active execution count, concurrency limit, and processing state.
     */
    ...
    /**
     * Stops task processing and cancels all active coroutines, releasing resources used by the TaskExecutionManager.
     */
    fun cleanup() {
