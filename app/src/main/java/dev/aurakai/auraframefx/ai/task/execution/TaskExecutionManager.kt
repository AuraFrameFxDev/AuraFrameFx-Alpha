...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for execution and assigns it to the most appropriate agent.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit preference, and enqueues the task for asynchronous processing.
     *
    ...
    /**
     * Returns the current execution status of the task with the specified ID, or `null` if the task is not found.
     *
    ...
    /**
     * Retrieves the result of a completed task by its unique ID, or returns null if the task is not found or has not completed.
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the completed task, or null if the task is unavailable or incomplete.
     */
     ...
     * @return A list of tasks matching the optional filters.
     */
     ...
     *
     * Introduces a short delay between processing cycles to avoid busy waiting, and a longer delay after errors to provide backoff.
     */
    ...
    /**
     * Processes the next eligible task from the queue if concurrency and scheduling allow.
     *
     * Polls the priority queue for the highest-priority task. If the task's scheduled time is in the future, it is re-queued. If concurrency limits are reached or the queue is empty, no task is processed. Eligible tasks are dispatched for execution.
     */
     ...
     *
     * @return The response produced by the Kai agent for the provided task.
     */
     ...
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the agent is chosen by analyzing keywords in the task type: Aura for "creative" or "ui", Kai for "security" or "analysis", and Genesis for "complex" or "fusion". Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Retrieves the number of tasks that are currently running.
     *
     ...
     *
     * @return The average execution time in milliseconds, or 0 if no tasks have completed.
     */
