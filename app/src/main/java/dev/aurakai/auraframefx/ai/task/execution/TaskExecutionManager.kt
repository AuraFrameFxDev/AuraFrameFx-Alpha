...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the completed task, or null if the task is not finished or does not exist.
     */
     ...
     *
     * @param status Optional filter to include only tasks with this execution status.
     * @param agentType Optional filter to include only tasks assigned to this agent type.
     ...
     *
     * The processor loops, invoking task processing with a short delay between cycles to minimize resource usage, and applies a longer delay if an error occurs to provide backoff.
     */
     ...
     *
     * Polls the priority queue for the highest-priority task and dispatches it for execution if its scheduled time has arrived and concurrency limits are not exceeded. If the task is not yet scheduled to run, it is re-queued.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's assigned agent type. Upon completion or failure, updates the task's result, execution statistics, and removes it from the set of active executions.
     *
     ...
     *
     * If an explicit agent preference is provided and recognized, it is used. Otherwise, the agent is selected by matching keywords in the task type, with Genesis as the default if no match is found.
     *
    ...
    /**
     * Refreshes the queue status state with current queue size, number of active executions, concurrency limit, and processing status.
     */
