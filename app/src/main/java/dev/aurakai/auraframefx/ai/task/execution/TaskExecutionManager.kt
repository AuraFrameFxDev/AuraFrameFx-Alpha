...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for asynchronous execution. Supports optional delayed scheduling.
     *
     ...
     * @return The execution status of the task, or `null` if not found.
     */
     ...
     *
     * Initiates execution of the highest-priority task whose scheduled time has arrived, provided the maximum number of concurrent tasks has not been reached. If the task is not yet due, it is re-enqueued for later processing.
     */
    ...
    /**
     * Determines the optimal agent type for executing a task, using agent preference if specified or routing by task type keywords.
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the agent is chosen based on keywords in the task type, defaulting to Genesis if no match is found.
     *
    ...
    /**
     * Updates the execution statistics state flow with current counts of total, completed, active, queued, and failed tasks, as well as the average execution time.
     */
    ...
    /**
     * Updates the queue status state flow with current metrics, including queue size, active executions, concurrency limit, and processing state.
     */
