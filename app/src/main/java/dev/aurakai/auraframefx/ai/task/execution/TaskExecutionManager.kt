...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for asynchronous execution, selecting the most appropriate agent based on task type or explicit preference.
     *
     ...
     *
     * Checks active, completed, and queued tasks to determine the most up-to-date status.
     *
     ...
     *
     * Removes the task from the queue if it is waiting, or marks an active task as cancelled to signal termination. Returns `true` if the task was found and cancelled; returns `false` if the task does not exist or is already completed.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results (converted to `TaskExecution`), and applies the specified filters.
     *
     * @param status If specified, only tasks with this execution status are included.
     * @param agentType If specified, only tasks executed by this agent type are included.
     ...
     *
     * The processor handles task execution, applies a short delay between tasks to prevent busy waiting, and uses a longer delay after errors to provide backoff.
     */
     ...
     *
     * Polls the priority queue for the next task. If the task's scheduled time is in the future, it is re-queued; otherwise, the task is executed immediately, provided the number of active executions is below the maximum allowed.
     */
    ...
    /**
     * Executes a task asynchronously using its assigned agent and updates its status and result upon completion or failure.
     *
     * Delegates the task to the appropriate agent for execution. On completion, records the result as successful or failed, updates execution statistics, and removes the task from active executions.
     *
     ...
     *
     * Builds an `AiRequest` from the task's type and data, then processes it with the Aura agent.
     *
     * @param execution The task execution containing the type and input data.
     * @return The response from the Aura agent for the executed task.
     */
     ...
     *
     * If the task specifies a valid agent preference, it is used. Otherwise, the agent is selected by analyzing keywords in the task type, with specific keywords mapped to Aura, Kai, or Genesis agents. Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Returns the current count of actively running tasks.
     *
    ...
    /**
     * Updates the queue status state flow with current information about the queue size, number of active executions, maximum concurrency, and processing state.
     */
    ...
    /**
     * Stops all task processing and cancels all running coroutines, releasing resources used by the TaskExecutionManager.
     */
    fun cleanup() {
