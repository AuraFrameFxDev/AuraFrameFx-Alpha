...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Returns the current execution status of the task with the specified ID.
     *
     * Searches active executions, completed results, and queued tasks to find the most recent status. Returns `null` if the task is not present in any state.
     *
     ...
     * @return List of tasks matching the provided filters.
     */
     ...
     *
     * The processor iterates with a short delay to minimize resource usage and applies a longer delay after exceptions to allow recovery.
     */
    ...
    /**
     * Processes the next pending task from the queue if concurrency limits allow.
     *
     * Polls the highest-priority task and, if its scheduled time has arrived, dispatches it for execution; otherwise, re-enqueues it for later processing.
     */
     ...
     *
     * Marks the task as running, delegates execution to the appropriate agent, and upon completion or failure, stores the outcome in the completed executions map. Updates execution statistics and queue status, and removes the task from active executions after processing.
     *
     ...
     *
     * @return The Kai agent's response for the specified task.
     */
    ...
    /**
     * Returns the number of tasks currently being executed asynchronously.
     *
