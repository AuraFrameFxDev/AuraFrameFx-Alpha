...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the completed task, or null if the task is not completed or does not exist.
     */
     ...
     *
     * The processor loops with a short delay between task processing cycles to minimize resource usage, and applies a longer delay if an error occurs to provide backoff.
     */
     ...
     *
     * Polls the priority queue for the highest-priority task. If the task's scheduled time is in the future, it is re-queued. If the concurrency limit is reached or the queue is empty, no task is processed. Eligible tasks are dispatched for asynchronous execution.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's configuration. Upon completion or failure, updates the task's result, execution statistics, and removes it from the set of active executions.
     *
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority before delegating execution to the Genesis agent.
     *
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is selected by matching keywords in the task type, with Genesis as the default if no match is found.
     *
    ...
    /**
     * Updates the internal queue status state to reflect the current queue size, number of active executions, concurrency limit, and processing state.
     */
