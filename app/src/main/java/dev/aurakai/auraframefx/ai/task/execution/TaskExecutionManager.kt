...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Removes the task from the queue if found, or marks an active task as cancelled. Returns `true` if the task was successfully cancelled, or `false` if the task does not exist or is already completed.
     *
     ...
     *
     * Introduces short delays between processing cycles to avoid busy waiting and applies longer delays after errors for backoff.
     */
     ...
     *
     * If the maximum number of concurrent tasks is reached or the queue is empty, no task is processed. Tasks scheduled for the future are re-queued. Tasks ready for execution are dispatched immediately.
     */
     ...
     *
     * Delegates the task to the appropriate agent for execution. Upon completion or failure, updates the task's result, execution statistics, and removes it from the list of active executions.
     *
