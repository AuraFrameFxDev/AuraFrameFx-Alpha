...
class TaskExecutionManager @Inject constructor(
     ...
     * @return `true` if the task was found and cancelled; `false` if no such task exists.
     */
     ...
     *
     * Polls the priority queue for a task whose scheduled time has arrived and, if the number of active executions is below the maximum allowed, initiates its execution. Tasks scheduled for the future are re-queued. If no eligible task is found or concurrency limits are reached, the function returns without processing.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's assignment. Upon completion or failure, stores the outcome in the completed executions map, updates execution statistics and queue status, and removes the task from active executions.
     *
     ...
     *
     * Converts the task's type and data into an `AiRequest`, then delegates processing to the Aura agent.
     *
     ...
     *
     * @param execution The task execution containing the details to build the Kai agent request.
     ...
     * @return The response generated by the Genesis agent for the given task.
     */
     ...
     *
     * If a valid agent preference is specified in the task execution, it is used. Otherwise, the method analyzes the task type string for keywords to determine the agent, defaulting to Genesis if no relevant keywords are found.
     *
     * @param execution The task execution metadata, including agent preference and task type.
     * @return The agent type assigned for executing the task.
     */
     ...
     *
     * @return The number of tasks in the active executions map.
     */
    ...
    /**
     * Refreshes the queue status state flow to reflect the current queue size, number of active executions, concurrency limit, and processing state.
     */
    ...
    /**
     * Stops the task processor and cancels all active coroutines, releasing resources held by the TaskExecutionManager.
     */
    fun cleanup() {
