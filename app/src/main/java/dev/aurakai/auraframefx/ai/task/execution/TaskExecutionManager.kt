...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Adds a short delay between processing cycles to minimize resource usage, and applies a longer delay after exceptions for error backoff.
     */
     ...
     *
     * Polls the priority queue for a task whose scheduled time has arrived. If the concurrency limit is reached or no task is ready, the function returns without processing. Tasks scheduled for the future are re-queued for later execution.
     */
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the function analyzes the task type string for keywords to determine the optimal agent, defaulting to Genesis if no match is found.
     *
     ...
     *
     * @return The number of tasks being processed concurrently.
     */
    ...
    /**
     * Refreshes the execution statistics state to reflect the current counts of total, completed, active, queued, and failed tasks, along with the average execution time.
     */
    ...
    /**
     * Computes the average execution time in milliseconds across all completed tasks.
     *
