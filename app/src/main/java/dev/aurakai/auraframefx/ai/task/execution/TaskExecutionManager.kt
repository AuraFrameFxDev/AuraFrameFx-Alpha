...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for execution, automatically selecting the optimal AI agent unless a preference is specified.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the appropriate agent, and enqueues the task for asynchronous processing.
     *
     * @param type The category or type of the task to execute.
     ...
     *
     * Removes the task from the queue or marks an active task as cancelled, signaling its execution to terminate. Returns `true` if cancellation succeeds, or `false` if the task is not found.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results (converted to `TaskExecution` objects). Filters can be applied to return only tasks with a specific status or executed by a specific agent type.
     *
     ...
     *
     * Delegates execution to the appropriate agent based on the task's agent assignment. Upon completion or failure, records the outcome in the completed executions map, updates execution statistics, and removes the task from the active executions list.
     *
     ...
     *
     * Constructs an agent request from the task execution details and returns the Kai agent's response.
     *
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by analyzing keywords in the task type, with a default to Genesis if no match is found.
     *
     * @param execution The task execution metadata used to determine agent routing.
    ...
    /**
     * Recalculates and updates the current execution statistics, including task counts and average execution time.
     *
     * Updates the internal state with the latest totals for active, completed, queued, and failed tasks, as well as the average execution time of completed tasks.
     */
    ...
    /**
     * Stops all task processing and cancels running coroutines, releasing resources held by the TaskExecutionManager.
     */
    fun cleanup() {
