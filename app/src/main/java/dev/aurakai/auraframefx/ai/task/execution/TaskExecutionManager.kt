...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for execution, assigning it to the optimal AI agent based on task type or explicit preference.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the specified parameters, determines the appropriate agent if not provided, and enqueues the task for asynchronous processing.
     *
    ...
    /**
     * Cancels a task that is either queued or currently running.
     *
     * Removes the task from the queue if present, or marks an active task as cancelled to signal termination. Returns `true` if the task was found and cancelled, or `false` if the task was not found.
     *
     ...
     *
     * Polls the priority queue for the next scheduled task, re-queues it if its scheduled time is in the future, or initiates its execution if ready and concurrency limits allow.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's agent assignment. Upon completion or failure, records the outcome in the completed executions, updates execution statistics, and removes the task from the active executions list.
     *
     ...
     *
     * Constructs an `AiRequest` from the task's type and data, then delegates processing to the Aura agent.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task execution data and delegates processing to the Kai agent.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task execution data and delegates processing to the Genesis agent.
     *
     * @param execution The task execution to be processed.
     ...
     *
     * If a valid agent preference is specified in the task, that agent is chosen. Otherwise, the agent is determined by analyzing keywords in the task type, with specific keywords mapped to Aura, Kai, or Genesis agents. Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Stops background task processing and cancels all active coroutines managed by the TaskExecutionManager.
     *
     * Releases resources and halts further task execution or scheduling.
     */
    fun cleanup() {
