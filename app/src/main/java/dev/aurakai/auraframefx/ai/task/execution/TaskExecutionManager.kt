...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Retrieves the current execution status of a task by its ID, or `null` if the task does not exist.
     *
     ...
     *
     * The processor iterates with a short delay to reduce resource usage and applies a longer delay after encountering errors to provide backoff.
     */
     ...
     *
     * Polls the highest-priority task and checks if its scheduled time has arrived. If not, the task is re-queued. If eligible, the task is dispatched for execution, provided the number of active executions is below the concurrency limit.
     */
     ...
     *
     * Delegates the task to the appropriate agent based on its assignment. Upon completion or failure, updates the task's result, execution statistics, and removes it from the set of active executions.
     *
    ...
    /**
     * Updates the queue status state with the latest queue size, number of active executions, concurrency limit, and processing flag.
     */
