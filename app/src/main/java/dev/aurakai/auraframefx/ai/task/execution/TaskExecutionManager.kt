...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new task for execution, assigning it to the most suitable agent based on task type or explicit preference.
     *
     ...
     *
     * Searches active, completed, and queued tasks to determine the status. Returns `null` if the task is not found in any state.
     *
     ...
     *
     * Removes the task from the queue or marks an active task as cancelled, signaling its execution to terminate. Returns `true` if the task was found and cancelled, or `false` if the task was not found.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results (converted to `TaskExecution`), then applies the specified filters.
     *
    ...
    /**
     * Launches a background coroutine to process tasks from the queue as long as processing is enabled.
     *
     * The processor executes pending tasks, applies a short delay between iterations, and uses a longer delay if an error occurs.
     */
     ...
     *
     * Polls the priority queue for the next scheduled task, re-queues it if its scheduled time is in the future, and initiates execution if it is ready and concurrency limits have not been reached.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's agent assignment. Upon completion or failure, records the result or error, updates execution statistics, and removes the task from the list of active executions.
     *
     ...
     *
     * Constructs an `AiRequest` from the task's data and type, then delegates processing to the Aura agent.
     *
     * @param execution The task execution containing input data and metadata.
     * @return The result returned by the Aura agent after processing the request.
     */
     ...
     *
     * Constructs an `AgentRequest` from the task execution details, including query, type, context, and priority metadata, and delegates processing to the Kai agent.
     *
    ...
    /**
     * Executes a task by delegating it to the Genesis agent.
     *
     * Constructs an `AgentRequest` from the task execution details and returns the agent's response.
     *
    ...
    /**
     * Selects the most appropriate agent for a task based on explicit agent preference or keywords in the task type.
     *
     * If a valid agent preference is specified, it is used. Otherwise, the agent is chosen by analyzing keywords in the task type, with "creative" or "ui" selecting Aura, "security" or "analysis" selecting Kai, and "complex" or "fusion" selecting Genesis. Defaults to Genesis if no keywords match.
     *
     * @param execution The task execution metadata used for agent selection.
     * @return The determined agent type for executing the task.
     */
    ...
    /**
     * Refreshes the execution statistics with the latest counts of total, completed, active, queued, and failed tasks, and updates the average execution time.
     */
    ...
    /**
     * Refreshes the queue status state with current metrics including queue size, active executions, maximum concurrency, and processing flag.
     */
    ...
    /**
     * Stops all task processing and cancels running coroutines, releasing resources used by the manager.
     */
    fun cleanup() {
...
class TaskPriorityComparator : Comparator<TaskExecution> {
    /**
     * Defines the ordering of TaskExecution objects for use in a priority queue.
     *
     ...
     *
     * @return A negative integer if the first task should precede the second, a positive integer if it should follow, or zero if they are considered equal.
     */
