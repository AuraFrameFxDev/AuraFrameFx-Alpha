...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for asynchronous execution by selecting the most appropriate AI agent.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for immediate or delayed execution.
     *
     ...
     *
     * Checks active, completed, and queued tasks to determine the latest status.
     *
     ...
     *
     * The processor attempts to execute queued tasks with a brief delay between each attempt. If an exception occurs, it waits longer before retrying. Processing continues until explicitly stopped.
     */
    ...
    /**
     * Processes the next eligible task from the queue if concurrency limits have not been reached.
     *
     * Retrieves the highest-priority task whose scheduled time has arrived and initiates its execution. If the task is not yet due, it is re-enqueued for future processing. No action is taken if the maximum number of concurrent tasks is already running or if the queue is empty.
     */
     ...
     * @return The agent type selected for processing the task.
     */
     ...
     *
     * @return The number of active (running) tasks.
     */
