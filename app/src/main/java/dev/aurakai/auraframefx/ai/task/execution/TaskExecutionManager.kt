...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Searches active, completed, and queued tasks to determine the latest status.
     *
     ...
     *
     * The processor runs in a loop, invoking task execution with a short delay between cycles to minimize resource usage. If an exception occurs, it applies a longer delay before retrying.
     */
     ...
     *
     * Polls the priority queue for the highest-priority task. If the task's scheduled time is in the future, it is re-queued. If the maximum number of concurrent tasks is already running or the queue is empty, no task is processed. Eligible tasks are dispatched for execution.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's configuration. Upon completion or failure, updates the task's status, stores the result, and refreshes execution statistics and queue status.
     *
     ...
     *
     * @return The response generated by the Kai agent for the specified task.
     */
    ...
    /**
     * Refreshes the queue status state to reflect the current queue size, number of active executions, concurrency limit, and processing activity.
     */
    ...
    /**
     * Stops task processing and cancels all active coroutines, releasing resources held by the TaskExecutionManager.
     */
    fun cleanup() {
