...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new task for execution, automatically selecting the most suitable agent unless a preference is specified.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided type, data, priority, and optional agent preference or scheduled time, and enqueues it for background processing. The agent is determined based on explicit preference or by analyzing the task type.
     *
     * @param type The category or description of the task to execute.
     * @param data The input parameters required for the task.
     * @param priority The execution priority for the task (default is NORMAL).
     * @param agentPreference Optional agent name to explicitly route the task; if omitted, routing is determined automatically.
     ...
     *
     * Handles task execution, applies a short delay between iterations to avoid busy waiting, and uses a longer delay after errors.
     */
     ...
     *
     * Polls the priority queue for the next eligible task, checks if its scheduled time has arrived, and initiates execution if ready. If the task is scheduled for the future, it is re-queued.
     */
     ...
     *
     * Delegates execution to the appropriate agent based on the task's agent assignment. Upon completion or failure, updates the task's status, stores the result, and refreshes execution statistics and queue status.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task execution details and delegates processing to the Kai agent.
     *
    ...
    /**
     * Selects the most appropriate agent for a task based on agent preference or keywords in the task type.
     *
     * If a valid agent preference is specified, it is used. Otherwise, the agent is determined by analyzing keywords in the task type, with a default to Genesis if no match is found.
     *
    ...
    /**
     * Refreshes the queue status to reflect the latest queue size, active executions, concurrency limit, and processing state.
     */
