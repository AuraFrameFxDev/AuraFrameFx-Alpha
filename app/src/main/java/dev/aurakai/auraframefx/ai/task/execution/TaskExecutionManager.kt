...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent if not explicitly provided, and enqueues the task for processing.
     *
     ...
     *
     * Searches active, completed, and queued tasks for the specified ID and returns the corresponding status, or `null` if the task does not exist.
     *
    ...
    /**
     * Returns the result of a completed task by its unique ID, or null if the task is not found or not completed.
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the completed task, or null if unavailable.
     */
     ...
     *
     * Attempts to remove the task from the queue or mark an active task as cancelled. Returns `true` if the task was found and cancellation was initiated, or `false` if the task does not exist.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results. Returns only those tasks that match the specified status and/or agent type if provided.
     *
     * @param status Optional filter to include only tasks with the given execution status.
     ...
     *
     * The processor executes tasks, applies a short delay between iterations to reduce CPU usage, and uses a longer delay after encountering errors to provide backoff.
     */
     ...
     *
     * Polls the priority queue for the next scheduled task. If the task's scheduled time has not yet arrived, it is re-queued. If the maximum number of concurrent tasks is already running or the queue is empty, no action is taken. Otherwise, the task is executed.
     */
    ...
    /**
     * Executes the given task asynchronously using its assigned agent and records the result or failure.
     *
     * Delegates execution to the appropriate agent based on the task's agent assignment. Upon completion or failure, updates the task's result, execution statistics, and removes it from the list of active executions.
     *
     ...
     *
     * @param execution The task execution details to process.
     ...
     *
     * Constructs an `AgentRequest` from the task's type, data, and priority, then delegates execution to the Kai agent.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task's type, data, and priority, then delegates execution to the Genesis agent.
     *
     * @param execution The task execution details.
     ...
     *
     * If a valid agent preference is provided, it is used. Otherwise, the agent is chosen by analyzing keywords in the task type, with a default to Genesis if no keywords match.
     *
     * @param execution The task execution metadata used for agent selection.
     * @return The agent type assigned to execute the task.
     */
