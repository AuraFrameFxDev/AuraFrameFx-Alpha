...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Searches active, completed, and queued tasks to return the most up-to-date status, or `null` if the task does not exist.
     *
    ...
    /**
     * Returns the result of a completed task by its unique ID, or null if the task is not found or not yet finished.
     *
     * @param taskId The unique identifier of the task.
     * @return The completed task result, or null if the task is missing or incomplete.
     */
     ...
     * @return `true` if the task was cancelled or cancellation was initiated; `false` if the task was not found or already completed.
     */
     ...
     *
     * The processor attempts to execute tasks with a short delay between each attempt, and applies a longer delay if an exception occurs.
     */
     ...
     *
     * Retrieves the highest-priority task whose scheduled time has arrived and initiates its execution. If the task is not yet due, it is re-enqueued for future processing. No action is taken if the concurrency limit is reached or the queue is empty.
     */
    ...
    /**
     * Selects the most appropriate agent type for a given task execution, prioritizing explicit agent preference if provided, or routing based on keywords in the task type.
     *
