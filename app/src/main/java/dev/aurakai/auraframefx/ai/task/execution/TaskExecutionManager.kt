...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Searches active, completed, and queued tasks to determine the most up-to-date status.
     *
     ...
     *
     * The processor iterates with a short delay to minimize resource usage and applies a longer delay after exceptions.
     */
     ...
     *
     * Polls the highest-priority pending task and, if its scheduled time has arrived, dispatches it for execution; otherwise, re-enqueues it for later processing.
     */
    ...
    /**
     * Runs the specified task asynchronously using its designated AI agent and records the result.
     *
     * Updates the task's status to running, delegates execution to the appropriate agent, and upon completion or failure, stores the outcome in the completed executions map. Removes the task from active executions and updates execution statistics and queue status after processing.
     *
    ...
    /**
     * Determines the most suitable agent type for processing a task, prioritizing explicit agent preference if provided, or selecting based on keywords in the task type.
     *
     * If the task specifies a valid agent preference, that agent is used. Otherwise, the agent is chosen by matching keywords in the task type, defaulting to Genesis if no match is found.
     *
    ...
    /**
     * Returns the number of tasks currently running asynchronously.
     *
