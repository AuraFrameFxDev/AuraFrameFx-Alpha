...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * The processor runs in a loop, invoking task processing with a brief delay between cycles to minimize resource usage, and applies a longer delay if an error occurs to provide backoff.
     */
    ...
    /**
     * Runs the specified task asynchronously using its designated AI agent and records the result or failure.
     *
     * Delegates execution to the assigned agent type. Upon completion or error, updates the task's result, execution statistics, and removes it from active executions.
     *
    ...
    /**
     * Updates the queue status state with the current queue size, number of active executions, concurrency limit, and processing status.
     */
