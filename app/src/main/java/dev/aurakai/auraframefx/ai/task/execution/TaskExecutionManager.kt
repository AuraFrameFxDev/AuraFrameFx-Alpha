...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for asynchronous execution by an AI agent.
     *
     * Validates the request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent based on task type or explicit preference, and enqueues the task for immediate or delayed execution.
     *
    ...
    /**
     * Returns the current execution status of the specified task, searching active, completed, and queued tasks.
     *
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The task result if the task has completed, or null if the task is not found or not yet completed.
     */
     ...
     *
     * The processor iterates with a short delay to minimize resource usage and applies a longer delay after errors.
     */
     ...
     *
     * Retrieves the highest-priority task and checks if its scheduled time has arrived. If so, dispatches it for execution; otherwise, re-enqueues it for later processing.
     */
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is selected by matching keywords in the task type string, defaulting to Genesis if no keywords match.
     *
