...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Returns the result of a completed task for the given task ID, or null if the task is not found or not yet completed.
     *
     * @param taskId The unique identifier of the task.
     ...
     *
     * The processor introduces a short delay between cycles to minimize CPU usage and applies a longer delay after encountering errors to provide backoff.
     */
    ...
    /**
     * Attempts to execute the next task in the queue if concurrency limits permit and the task's scheduled time has arrived.
     *
     * If the maximum number of concurrent tasks is reached, or if the next task is not yet due, no execution occurs. Tasks not ready for execution are re-enqueued for future processing.
     */
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by matching keywords in the task type, with Genesis as the default if no match is found.
     *
    ...
    /**
     * Refreshes the execution statistics state flow with the latest counts of total, completed, active, queued, and failed tasks, along with the current average execution time.
     */
