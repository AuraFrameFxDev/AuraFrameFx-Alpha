...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent if not provided, and enqueues the task for background processing. Supports optional priority and delayed execution.
     *
     * @param type The category or identifier of the task to execute.
     * @param data The input data required for the task.
     * @param priority The priority level for execution (default is NORMAL).
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The result of the task if it has completed, or null if the task is not found or not yet completed.
     */
    ...
    /**
     * Launches a background coroutine that continuously processes tasks from the queue while processing is active.
     *
     * The processor executes pending tasks, applies a short delay between iterations to avoid busy waiting, and uses a longer delay after errors to provide backoff.
     */
    ...
    /**
     * Executes a task by sending it to the Aura agent for processing.
     *
     * Constructs an `AiRequest` from the task's type and data, then delegates execution to the Aura agent.
     *
    ...
    /**
     * Executes a task by delegating it to the Kai agent.
     *
     * Constructs an `AgentRequest` from the task execution details and returns the Kai agent's response.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task execution details and delegates processing to the Genesis agent.
     *
     * @param execution The task execution to be processed.
    ...
    /**
     * Selects the most appropriate agent for a task based on agent preference or task type analysis.
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by matching keywords in the task type, with a default to Genesis if no match is found.
     *
    ...
    /**
     * Refreshes the execution statistics state with the latest counts of total, completed, active, queued, and failed tasks, and updates the average execution time.
     */
    ...
    /**
     * Updates the queue status state flow with the latest queue size, active executions count, maximum concurrency, and processing state.
     */
    ...
    /**
     * Stops all task processing and cancels active coroutines, releasing resources held by the TaskExecutionManager.
     */
    fun cleanup() {
