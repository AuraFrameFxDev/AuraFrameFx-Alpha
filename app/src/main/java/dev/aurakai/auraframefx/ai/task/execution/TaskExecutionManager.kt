...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Removes the task from the queue if it has not started, or marks an active task as cancelled so its execution can be interrupted. Returns `true` if the task was found and cancellation was initiated, or `false` if the task does not exist or has already completed.
     *
     ...
     *
     * Applies a short delay between processing cycles to prevent busy waiting, and a longer delay after errors to provide backoff.
     */
     ...
     *
     * Polls the priority queue for a task. If the task's scheduled time is in the future, it is re-queued; otherwise, the task is executed. No action is taken if the concurrency limit is reached or the queue is empty.
     */
    ...
    /**
     * Executes the specified task with the Aura agent and returns the agent's response.
     *
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, then delegates execution to the Kai agent.
     *
     ...
     *
     * Builds an `AgentRequest` from the task's type and data, including priority metadata, and delegates processing to the Genesis agent.
     *
     * @param execution The task execution to process.
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is selected by analyzing keywords in the task type string, with a default to Genesis if no keywords match.
     *
    ...
    /**
     * Retrieves the number of tasks currently in execution.
     *
    ...
    /**
     * Refreshes the queue status to reflect the latest queue size, number of active executions, concurrency limit, and processing state.
     */
