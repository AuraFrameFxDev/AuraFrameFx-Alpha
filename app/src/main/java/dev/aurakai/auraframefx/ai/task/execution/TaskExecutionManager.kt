...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution`, determines the optimal agent if not specified, and enqueues the task for asynchronous processing.
     *
     ...
     *
     * Removes the task from the queue if it is pending, or marks an active task as cancelled. Returns `true` if the task was found and cancellation was initiated, or `false` if the task does not exist or is already completed.
     *
     ...
     *
     * If the number of active executions is below the concurrency limit, polls the highest-priority task from the queue. If the task's scheduled time is in the future, it is re-queued. Otherwise, dispatches the task for execution.
     */
     ...
     *
     * @param execution The task execution details, including type and data.
     ...
     *
     * Converts the task's data and metadata into an `AgentRequest` and delegates processing to the Genesis agent.
     *
    ...
    /**
     * Determines the most suitable agent type for executing a task, using agent preference if provided or routing based on task type keywords.
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the agent is chosen by matching keywords in the task type to known agent capabilities. Defaults to Genesis if no keywords match.
     *
     ...
     *
     * @return The number of tasks that are currently being executed.
     */
