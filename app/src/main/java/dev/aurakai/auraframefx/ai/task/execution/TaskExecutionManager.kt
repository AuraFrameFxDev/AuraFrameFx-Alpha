...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Removes the task from the queue or marks an active task as cancelled, allowing its execution to be interrupted. Returns `true` if the task was found and cancelled, or `false` if no such task exists.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results (converted to `TaskExecution` objects). If filters are provided, only tasks matching the specified status and/or agent type are returned.
     *
     ...
     *
     * The processor applies a short delay between task processing cycles and a longer delay after encountering errors to manage resource usage and provide error backoff.
     */
     ...
     *
     * Polls the priority queue for the next eligible task. If the task's scheduled time is in the future, it is re-queued; otherwise, the task is executed. Skips processing if the maximum number of concurrent tasks is already running or if the queue is empty.
     */
     ...
     *
     * Constructs an `AgentRequest` from the task's type and data, then delegates processing to the Kai agent.
     *
     * @param execution The task to be executed.
     ...
     *
     * Constructs an `AgentRequest` from the task's type and data, then processes it with the Genesis agent.
     *
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by analyzing keywords in the task type: "creative" or "ui" selects Aura, "security" or "analysis" selects Kai, "complex" or "fusion" selects Genesis. Defaults to Genesis if no keywords match.
     *
     * @param execution The task execution metadata used for agent selection.
     * @return The agent type chosen for executing the task.
     */
    ...
    /**
     * Updates the current queue status with the latest queue size, active executions, maximum concurrency, and processing state.
     */
     ...
     *
     * If no tasks have been completed, returns 0.
     *
