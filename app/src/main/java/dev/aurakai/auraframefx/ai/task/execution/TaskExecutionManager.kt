...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Introduces a short delay between processing cycles to reduce resource consumption, and a longer delay after exceptions to provide error backoff.
     */
    ...
    /**
     * Processes the next task from the queue if concurrency limits allow.
     *
     * Polls the priority queue for a task that is ready to execute based on its scheduled time. If the task is not yet scheduled to run, it is re-queued. If the maximum number of concurrent tasks is already running or the queue is empty, no task is processed.
     */
     ...
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the function analyzes the task type for keywords to intelligently route the task to an agent, defaulting to Genesis if no match is found.
     *
