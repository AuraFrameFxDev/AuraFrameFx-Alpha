...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for asynchronous processing.
     *
     ...
     *
     * Applies a short delay between processing cycles to reduce CPU usage, and a longer delay after errors to provide backoff.
     */
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, and delegates processing to the Kai agent.
     *
     * @param execution The task execution to be processed by the Kai agent.
     * @return The response generated by the Kai agent for the task.
     */
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, and delegates processing to the Genesis agent.
     *
     ...
     *
     * Uses the specified agent preference if provided and valid; otherwise, selects an agent by analyzing keywords in the task type. Defaults to Genesis if no preference or keyword match is found.
     *
     ...
     *
     * @return The number of tasks that are currently in execution.
     */
