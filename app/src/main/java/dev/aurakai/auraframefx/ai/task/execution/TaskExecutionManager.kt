...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for execution, routing it to the most appropriate agent based on task type or explicit agent preference.
     *
     ...
     *
     * If the maximum number of concurrent tasks is reached or the queue is empty, no task is processed. Tasks scheduled for the future are re-queued. Only tasks ready for execution are dispatched.
     */
     ...
     *
     * Builds an `AgentRequest` from the task's type, data, and priority, and processes it with the Kai agent.
     *
     ...
     *
     * Call this method to gracefully shut down the manager and halt all background task execution.
     */
    fun cleanup() {
