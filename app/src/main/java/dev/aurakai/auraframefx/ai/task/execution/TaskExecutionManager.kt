...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Removes the task from the queue if it is pending, or marks it as cancelled if it is actively executing. Returns `true` if the task was found and cancellation was initiated, or `false` if the task does not exist or is already completed.
     *
     ...
     *
     * Introduces a short delay between processing cycles to reduce resource usage, and applies a longer delay after errors to provide backoff.
     */
     ...
     *
     * If the number of active executions is below the concurrency limit and a queued task is ready to run (its scheduled time has arrived), the task is dispatched for execution. Tasks not yet ready are re-queued. No action is taken if the queue is empty or concurrency limits are reached.
     */
    ...
    /**
     * Shuts down the TaskExecutionManager by stopping task processing and cancelling all running coroutines.
     *
