...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * @param status If provided, includes only tasks with this execution status.
     * @param agentType If provided, includes only tasks assigned to this agent type.
     ...
     *
     * Polls the priority queue for a task whose scheduled time has arrived. If the task is not yet ready, it is re-queued. If the concurrency limit is reached or the queue is empty, the function exits without processing a task.
     */
     ...
     *
     * If a valid agent preference is specified, it is used. Otherwise, selects the agent by analyzing keywords in the task type, defaulting to Genesis if no match is found.
     *
    ...
    /**
     * Stops task processing, cancels all active coroutines, and releases resources used by the TaskExecutionManager.
     */
    fun cleanup() {
