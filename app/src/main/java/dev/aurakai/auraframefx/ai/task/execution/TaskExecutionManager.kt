...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task for asynchronous execution, selecting the most suitable agent based on task type or explicit agent preference.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent if not specified, and enqueues the task for processing.
     *
     ...
     *
     * @param status If provided, limits results to tasks with this execution status.
     * @param agentType If provided, limits results to tasks assigned to this agent type.
     * @return List of tasks matching the specified filters.
     */
     ...
     *
     * Introduces a short delay between processing cycles to reduce resource usage, and a longer delay after errors to provide backoff.
     */
     ...
     *
     * If the maximum number of concurrent tasks is reached or the queue is empty, no task is processed. Tasks scheduled for the future are re-queued. Tasks ready for execution are dispatched for processing.
     */
     ...
     *
     * Delegates the task to the appropriate agent for execution. Upon completion or failure, stores the result, updates execution statistics, and removes the task from the list of active executions.
     *
     ...
     *
     * If the task specifies a valid agent preference, that agent type is selected. Otherwise, the agent is chosen by analyzing keywords in the task type: Aura for "creative" or "ui", Kai for "security" or "analysis", and Genesis for "complex" or "fusion". Defaults to Genesis if no keywords match.
     *
     * @param execution The task execution metadata used to select the agent.
    ...
    /**
     * Refreshes the queue status state flow with current metrics such as queue size, number of active executions, maximum concurrency, and processing state.
     */
