...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task and assigns it to the most suitable AI agent for asynchronous execution.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for processing. Supports optional priority and delayed scheduling.
     *
     ...
     *
     * Searches active, completed, and queued tasks for the specified ID. Returns the corresponding status, or `null` if the task does not exist.
     *
    ...
    /**
     * Processes the next eligible task from the queue, respecting concurrency and scheduling constraints.
     *
     * If the maximum number of concurrent tasks is reached, or if the queue is empty, no task is processed. Tasks scheduled for the future are re-queued. Tasks ready for execution are dispatched asynchronously.
     */
     ...
     *
     * Converts the task execution details into an `AgentRequest` and delegates processing to the Kai agent.
     *
     * @param execution The task execution to be processed by the Kai agent.
     * @return The response generated by the Kai agent for the given task.
     */
     ...
     *
     * Converts the task's type, data, and priority into an `AgentRequest` and delegates processing to the Genesis agent.
     *
     * @param execution The task execution to process.
     ...
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the agent is chosen by matching keywords in the task type: "creative" or "ui" selects Aura, "security" or "analysis" selects Kai, and "complex" or "fusion" selects Genesis. Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Updates the queue status state flow with current queue size, number of active executions, maximum concurrency, and processing state.
     */
