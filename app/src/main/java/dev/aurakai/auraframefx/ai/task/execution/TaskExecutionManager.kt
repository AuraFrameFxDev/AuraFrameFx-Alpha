...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task and assigns it to the most suitable AI agent for execution.
     *
     * Validates the request, creates a pending `TaskExecution` with the specified parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for asynchronous processing. The task will be executed after the optional scheduled time, or immediately if not specified.
     *
     ...
     * @return The execution status if the task is found, or `null` if not found.
     */
     ...
     *
     * @param taskId The unique identifier of the task.
     * @return The completed task result, or null if the task does not exist or is not yet completed.
     */
     ...
     *
     * Introduces a brief delay between processing cycles to prevent busy waiting, and applies a longer delay after encountering errors to provide backoff.
     */
     ...
     *
     * If the maximum number of concurrent tasks is reached or the queue is empty, no action is taken. Tasks scheduled for the future are re-queued. Otherwise, the highest-priority ready task is dispatched for execution.
     */
     ...
     *
     * If a valid agent preference is specified in the task, it is used. Otherwise, the agent is determined by analyzing keywords in the task type: Aura for "creative" or "ui", Kai for "security" or "analysis", and Genesis for "complex" or "fusion". Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Refreshes the queue status with the latest metrics, including queue size, active executions, concurrency limit, and processing state.
     */
