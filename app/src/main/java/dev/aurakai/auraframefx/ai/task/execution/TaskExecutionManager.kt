...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the scheduling request, creates a pending `TaskExecution`, determines the optimal agent if not explicitly specified, and enqueues the task for processing.
     *
     ...
     *
     * Continuously invokes task processing with a short delay between cycles to avoid busy waiting. If an error occurs, applies a longer delay before retrying.
     */
     ...
     *
     * Uses the agent preference if specified and valid; otherwise, selects an agent by analyzing keywords in the task type. Defaults to Genesis if no preference or keyword match is found.
     *
