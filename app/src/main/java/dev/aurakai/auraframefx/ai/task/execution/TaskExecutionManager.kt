...
class TaskExecutionManager @Inject constructor(
    ...
    /**
     * Schedules a new background task and assigns it to the most appropriate AI agent.
     *
     * Validates the scheduling request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit preference, and enqueues the task for asynchronous execution.
     *
     ...
     *
     * Aggregates tasks from the queue, active executions, and completed results. Converts completed results back to `TaskExecution` objects with status set to COMPLETED.
     *
    ...
    /**
     * Starts a background coroutine that processes tasks from the queue as long as processing is enabled.
     *
     * Introduces a short delay between cycles to avoid busy waiting and a longer delay after errors for backoff.
     */
     ...
     *
     * If the maximum number of concurrent tasks is reached, or if the queue is empty, no action is taken. Tasks scheduled for the future are re-queued. Tasks ready for execution are dispatched.
     */
     ...
     *
     * Delegates the task to the appropriate agent based on its assignment. Upon completion or failure, updates the task's result, execution statistics, and removes it from the list of active executions.
     *
     ...
     *
     * Builds an `AgentRequest` from the task's details and delegates processing to the Kai agent.
     *
    ...
    /**
     * Determines the optimal agent type to execute a task based on agent preference or task type keywords.
     *
     * If the task specifies a valid agent preference, that agent is selected. Otherwise, the agent is chosen by matching keywords in the task type to predefined categories, defaulting to Genesis if no match is found.
     *
