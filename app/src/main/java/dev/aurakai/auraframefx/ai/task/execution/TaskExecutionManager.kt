...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Validates the request, creates a pending `TaskExecution` with the provided parameters, determines the optimal agent based on task type or explicit agent preference, and enqueues the task for processing.
     *
     ...
     *
     * Searches active, completed, and queued tasks for the specified task ID and returns its status, or `null` if the task does not exist.
     *
    ...
    /**
     * Returns the result of a completed task by its unique identifier, or null if the task is not found or incomplete.
     *
     * @param taskId The unique identifier of the task.
     ...
     *
     * Removes the task from the queue if it is pending, or marks it as cancelled if it is active. Returns `true` if cancellation was initiated; returns `false` if the task does not exist or has already completed.
     *
     ...
     *
     * The processor loop includes a short delay between cycles to reduce resource usage and applies a longer delay after encountering errors to provide backoff.
     */
     ...
     *
     * Polls the highest-priority task and checks if its scheduled time has arrived. If not, the task is re-queued. If eligible, the task is dispatched for execution.
     */
    ...
    /**
     * Executes a task asynchronously using its designated AI agent and records the outcome.
     *
     * Delegates task processing to the assigned agent type. Upon completion or failure, updates the task's result, execution statistics, and removes it from the set of active executions.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task's details and submits it to the Kai agent for processing.
     *
     ...
     *
     * Constructs an `AgentRequest` from the task's details and submits it to the Genesis agent for processing.
     *
     ...
     *
     * If the task specifies a valid agent preference, that agent is chosen. Otherwise, the agent is determined by matching keywords in the task type, defaulting to Genesis if no keywords match.
     *
    ...
    /**
     * Updates the queue status state flow with the latest queue size, number of active executions, concurrency limit, and processing status.
     */
