...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Polls the priority queue for a task. If the task's scheduled time is in the future, it is re-queued; otherwise, the task is executed. No action is taken if the maximum number of concurrent tasks is already running or if the queue is empty.
     */
     ...
     *
     * Builds an `AgentRequest` from the task's type and data, including priority metadata, and processes it with the Kai agent.
     *
     * @param execution The task execution details.
     * @return The Kai agent's response to the request.
     */
     ...
     *
     * Builds an `AgentRequest` from the task's type and data, including priority metadata, and processes it with the Genesis agent.
     *
     * @param execution The task execution to be processed by the Genesis agent.
     * @return The response produced by the Genesis agent for the given task.
     */
     ...
     *
     * If an agent preference is specified in the task and matches a known agent, it is used. Otherwise, the agent is selected by analyzing keywords in the task type, with specific keywords mapped to Aura, Kai, or Genesis. Defaults to Genesis if no keywords match.
     *
    ...
    /**
     * Updates the internal queue status to reflect the latest queue size, number of active executions, concurrency limit, and processing state.
     */
