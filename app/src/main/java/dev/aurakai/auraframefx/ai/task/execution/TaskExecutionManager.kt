...
class TaskExecutionManager @Inject constructor(
     ...
     *
     * Polls the priority queue for a task whose scheduled time has arrived. If the maximum number of concurrent tasks is already running or no task is ready, the function returns without processing. Tasks scheduled for a future time are re-queued for later execution.
     */
     ...
     *
     * @param execution The task execution containing details for the Kai agent request.
     ...
     *
     * @return The response from the Genesis agent for the specified task.
     */
     ...
     *
     * Uses the specified agent preference if valid; otherwise, analyzes the task type string for keywords to select an agent, defaulting to Genesis if no match is found.
     *
     ...
     *
     * @return The count of active tasks in execution.
     */
