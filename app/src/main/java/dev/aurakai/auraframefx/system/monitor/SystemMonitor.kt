...
class SystemMonitor @Inject constructor(
     ...
     *
     * Disables periodic metric updates and halts background monitoring tasks.
     */
    fun stopMonitoring() {
     ...
     *
     * The returned map includes CPU usage percentage, memory usage and availability in bytes, memory usage percentage,
     ...
     *
     * The score ranges from 0.0 (poor health) to 1.0 (optimal health), averaging an inverted CPU usage score (capped at 100%) and a minimum-threshold ratio of available memory to total memory.
     *
     * @return The current system health score, where higher values indicate better health.
     */
    ...
    /**
     * Determines whether the system is currently under stress based on CPU and memory thresholds.
     *
     * The system is considered under stress if any of the following conditions are met: CPU usage exceeds 80%, memory usage percentage exceeds 85%, or available memory is less than 50 MB.
     *
     ...
     *
     * The report includes CPU usage, memory usage and availability, memory usage percentage, network activity, system health score, stress status, process and thread counts, JVM heap size and usage, and a timestamp.
     *
     * @return A `SystemPerformanceReport` containing all monitored system metrics and status at the time of invocation.
     */
     ...
     *
     * This function suspends while refreshing all monitored system metrics.
     */
     ...
     *
     * Attempts to calculate CPU usage and update the corresponding state flow. If calculation fails, logs a warning and retains the previous value.
     */
    ...
    /**
     * Refreshes the available and used memory metrics by querying the system's current memory status.
     *
     * Updates internal state flows with the latest available and used memory values from the Android ActivityManager.
     * If memory information cannot be retrieved, previous metric values remain unchanged.
     */
