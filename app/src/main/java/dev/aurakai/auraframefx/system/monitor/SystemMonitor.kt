...
class SystemMonitor @Inject constructor(
     ...
     * network bytes received and transmitted, process ID, thread count, heap size and usage in bytes, and a timestamp.
     *
     * @param component The identifier for which metrics are being collected.
     ...
     *
     * The score ranges from 0.0 (poor health) to 1.0 (optimal health), averaging an inverted CPU usage ratio (capped at 100%) and a minimum-threshold available memory ratio.
     *
     ...
     *
     * The report includes CPU usage, memory usage, available memory, memory usage percentage, network activity, system health score, stress status, process ID, thread count, heap size, used heap, and a timestamp.
     *
    ...
    /**
     * Updates the available and used memory metrics by querying the system's current memory status.
     *
     * Retrieves memory information from the Android ActivityManager and updates the corresponding state flows.
    ...
    /**
     * Updates the network metrics state flow with placeholder values.
     *
     * Sets all network metrics to zero as a temporary implementation. Intended to be replaced with actual network monitoring logic in the future.
     */
     ...
     *
     * @return Memory usage percentage, or 0f if unavailable.
     */
