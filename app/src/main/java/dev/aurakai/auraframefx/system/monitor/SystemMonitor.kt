...
class SystemMonitor @Inject constructor(
     ...
     *
     * @return `true` if any of the stress conditions are met; otherwise, `false`.
     */
     ...
     *
     * The returned report includes CPU usage, memory usage and availability, memory usage percentage, network activity, system health score, stress status, process and thread counts, JVM heap size and usage, and a timestamp.
     *
    ...
    /**
     * Updates the current CPU usage metric by calculating and storing the latest value.
     *
     ...
     *
     * Retrieves available and used memory from the Android ActivityManager and updates the corresponding state flows.
     * If retrieval fails, logs a warning and leaves previous metric values unchanged.
     */
