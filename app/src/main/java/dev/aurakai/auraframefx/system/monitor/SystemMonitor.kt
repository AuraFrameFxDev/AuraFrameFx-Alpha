...
class SystemMonitor @Inject constructor(
     ...
     *
     * @param component The identifier for which metrics are collected.
    ...
    /**
     * Generates a detailed report of the current system performance metrics and status.
     *
     * The report includes CPU usage, memory usage, available memory, memory usage percentage, network metrics,
     * system health score, stress status, process ID, thread count, heap size, used heap, and a timestamp.
     *
     * @return A `SystemPerformanceReport` containing a snapshot of all key system metrics and status.
     */
    ...
    /**
     * Refreshes CPU usage, memory usage, and network activity metrics on a background thread.
     *
     * Suspends while updating all monitored system metrics using the IO dispatcher.
     */
    ...
    /**
     * Refreshes the CPU usage metric by recalculating and updating its value.
     *
     * If CPU usage calculation fails, retains the previous value and logs a warning.
     */
     ...
     *
     * If retrieval fails, logs a warning and retains the previous metric values.
     */
     ...
     *
     * This is a placeholder implementation; replace with actual CPU usage calculation for production use.
     *
    ...
    /**
     * Retrieves the current heap memory usage by the JVM in bytes.
     *
    ...
    /**
     * Stops monitoring and cancels all background tasks, releasing resources used by the system monitor.
     */
    fun cleanup() {
