...
class SystemMonitor @Inject constructor(
    ...
    /**
     * Initiates periodic system performance monitoring if not already active.
     *
     * Spawns a background coroutine that updates system metrics at the specified interval.
     *
     ...
     * @return A map with metric names as keys and their current values.
     */
     ...
     *
     * Returns `true` if CPU usage exceeds 80%, memory usage percentage exceeds 85%, or available memory is less than 50 MB.
     *
     * @return `true` if any stress condition is detected; otherwise, `false`.
     */
    ...
    /**
     * Suspends execution to update CPU usage, memory usage, and network activity metrics on a background thread.
     *
     * This method sequentially refreshes all monitored system metrics using the IO dispatcher.
     */
     ...
     *
     * Attempts to calculate CPU usage and update the corresponding state. If calculation fails, logs a warning and retains the previous CPU usage value.
     */
    ...
    /**
     * Updates internal memory usage metrics by querying the system's current memory information.
     *
     * Retrieves available and total memory from the Android ActivityManager and updates the corresponding state flows.
