...
class SecurityMonitor @Inject constructor(
    ...
    /**
     * Continuously monitors for active threat detection and periodically reports identified threats to Genesis.
     *
     * When threat detection is enabled, launches a coroutine that checks for suspicious activity every 30 seconds and reports any detected high-confidence threats.
     */
     ...
     *
     * Sends a security event to Genesis for each encryption status change, indicating the new status and its severity. If an encryption error is detected, also reports a threat detection event describing the encryption failure.
     */
     ...
     *
     * Returns a map containing details such as the number of active threats, the timestamp of the last scan, the current status, and a beta mode flag. In beta mode, mock data is provided. If retrieval fails, the map includes an error message.
     *
