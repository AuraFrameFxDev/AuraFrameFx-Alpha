...
class SecurityMonitor @Inject constructor(
    ...
    /**
     * Starts monitoring security state, threat detection, encryption status, and permissions asynchronously.
     *
     * Initializes the Genesis bridge service if available, launches monitoring coroutines for each security aspect, and activates Android-level threat detection. Has no effect if monitoring is already active.
     */
    ...
    /**
     * Continuously monitors the security state and reports changes as security events to Genesis.
     *
     * Collects updates from the security context, constructs a `SecurityEvent` reflecting the current error state and message, and sends it to Genesis for further processing.
     */
    ...
    /**
     * Continuously monitors for active threat detection and reports any identified high-confidence threats to Genesis every 30 seconds while monitoring is enabled.
     */
    ...
    /**
     * Monitors changes in encryption status and reports relevant events and threats to Genesis.
     *
     * For each encryption status update, sends an event to Genesis indicating the new status and its severity. If an encryption error is detected, also reports a threat detection event describing the encryption failure.
     */
     ...
     *
     * Identifies denied permissions from the current permissions state and sends a warning event to Genesis if any are found.
     */
     ...
     *
     * Detects threats such as repeated encryption failures and denial of multiple critical privacy permissions (camera, microphone, location). Returns a list of identified threats based on these patterns.
     *
     ...
     *
     * Serializes the provided event or threat data, constructs a Genesis request, and attempts to initialize communication with Genesis. In beta mode, actual transmission is stubbed. Handles and logs any serialization or communication errors.
     *
     ...
     *
     * Returns a map containing information such as the number of active threats, the timestamp of the last scan, the current security status, and a beta mode indicator. If retrieval fails, the map includes an error message.
     *
