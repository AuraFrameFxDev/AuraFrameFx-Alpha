...
class GenesisAgentTest {
    ...
    fun testIntegration_workflowWithRealWorldScenario() = runBlocking {
        ...
        assertEquals("Should have working agents only", 3, externalResponses.size)
    ...
    fun testErrorRecovery_resilientWorkflow() = runBlocking {
        ...
        // Should have critical agent plus reliable agents (7 reliable + 1 critical = 8)
        assertEquals("Should have working agents", 8, responses.size)
        assertTrue("Should have critical agent", responses.containsKey("CriticalAgent"))
        assertEquals("critical success", responses["CriticalAgent"]?.content)
        
        // Verify unreliable agents failed
        (1..10).forEach { i ->
            if (i % 3 == 0) {
                assertNull("UnreliableAgent$i should fail", responses["UnreliableAgent$i"])
            } else {
                assertTrue("ReliableAgent$i should succeed", responses.containsKey("ReliableAgent$i"))
            }
        }
    }

    @Test
    fun testSecurity_maliciousInputHandling() = runBlocking {
