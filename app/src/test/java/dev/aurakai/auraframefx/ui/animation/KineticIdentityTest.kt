...
class KineticIdentityTest {
    ...
    fun kineticIdentity_allSwipeDirections_capturePositions() {
        // Arrange
        var rightSwipePositions = 0
        var leftSwipePositions = 0
        var upSwipePositions = 0
        var downSwipePositions = 0

        // Act & Assert for each direction
        listOf(
            { composeTestRule.onNodeWithTag("kinetic_component").performTouchInput { swipeRight() }; "right" },
            { composeTestRule.onNodeWithTag("kinetic_component").performTouchInput { swipeLeft() }; "left" },
            { composeTestRule.onNodeWithTag("kinetic_component").performTouchInput { swipeUp() }; "up" },
            { composeTestRule.onNodeWithTag("kinetic_component").performTouchInput { swipeDown() }; "down" }
        ).forEach { swipeAction ->
            val capturedPositions = mutableListOf<Offset>()
            val onPositionChange: (Offset) -> Unit = { position ->
                capturedPositions.add(position)
            ...
            assertTrue("${swipeAction()} should capture positions", capturedPositions.isNotEmpty())
        }
    }
