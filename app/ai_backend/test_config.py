...
def create_mock_response(status_code=200, json_data=None, headers=None, text=None):
    """
    Return a mock HTTP response object with configurable status code, headers, text, and JSON data for testing purposes.
    
    Parameters:
    	status_code (int, optional): HTTP status code to set on the mock response. Defaults to 200.
    	json_data (any, optional): Data to return when the mock's .json() method is called. If not provided, .json() raises a ValueError.
    	headers (dict, optional): Headers to include in the mock response. Defaults to an empty dictionary.
    	text (str, optional): Text content of the mock response. Defaults to an empty string.
    
    Returns:
    	Mock: A unittest.mock.Mock object simulating an HTTP response, including .status_code, .headers, .text, .json(), and .raise_for_status().
    """
...
def create_test_connector(config_overrides=None):
    """
    Instantiate and return a GenesisConnector configured for testing, allowing optional overrides of default test settings.
