...
def create_mock_response(status_code=200, json_data=None, headers=None, text=None):
    ...
    The returned mock object simulates an HTTP response, including `.status_code`, `.headers`, `.text`, a `.json()` method (returns the provided JSON data or raises `ValueError` if none is set), and a `.raise_for_status()` method (raises an exception for status codes 400 or higher).
    
    Returns:
        Mock: A mock object representing an HTTP response for use in tests.
    """
...
def create_test_connector(config_overrides=None):
    """
    Create and return a GenesisConnector instance configured for testing.
    
    If configuration overrides are provided, they are merged with the default test configuration before initializing the connector.
    
    Parameters:
        config_overrides (dict, optional): Dictionary of configuration values to override the defaults.
    
    Returns:
        GenesisConnector: A test instance of GenesisConnector with the specified configuration.
    """
