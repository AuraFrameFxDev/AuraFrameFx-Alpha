...
def create_mock_response(status_code=200, json_data=None, headers=None, text=None):
    """
    Generate a mock HTTP response object with customizable status code, JSON data, headers, and text content for testing purposes.
    ...
    Parameters:
        ...
        headers (dict, optional): HTTP headers to include in the mock response.
        text (str, optional): Text content of the response.
    
    Returns:
        Mock: A mock response object simulating HTTP response behavior, including `.json()` and `.raise_for_status()` methods.
    """
