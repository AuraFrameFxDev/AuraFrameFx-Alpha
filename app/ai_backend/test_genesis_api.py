...
class TestGenesisAPIClientExtended:
    """Extended test suite for additional edge cases and scenarios."""
    
    @pytest.fixture
    def client_with_custom_timeout(self, mock_config):
        """Fixture providing a client with custom timeout configuration."""
        mock_config['timeout'] = 10
        return GenesisAPIClient(**mock_config)
    
    @pytest.fixture
    def client_with_zero_retries(self, mock_config):
        """Fixture providing a client with zero retry attempts."""
        mock_config['max_retries'] = 0
        return GenesisAPIClient(**mock_config)
    
    def test_client_initialization_with_invalid_base_url(self):
        """Test that client initialization with malformed base URL raises appropriate error."""
    ...
    @pytest.mark.asyncio
    async def test_chat_completion_with_empty_model_name(self, client, sample_messages):
        """Test chat completion with empty model name in config."""
        invalid_config = ModelConfig(name='')
    ...
    @pytest.mark.asyncio
    async def test_chat_completion_with_null_model_config(self, client, sample_messages):
        """Test chat completion with None model config."""
        with pytest.raises(ValidationError, match="Model config is required"):
            await client.create_chat_completion(
                messages=sample_messages,
                model_config=None
            )
    ...
    @pytest.mark.asyncio
    async def test_chat_completion_with_concurrent_session_closure(self, client, sample_messages, sample_model_config):
        """Test behavior when session is closed during request."""
        mock_response = {
    ...
    async def test_streaming_with_empty_chunks(self, client, sample_messages, sample_model_config):
        """Test streaming when receiving empty chunks."""
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            # Should process non-empty chunks correctly
            assert len(chunks) >= 1
    
    @pytest.mark.asyncio
    async def test_request_header_injection_protection(self, client, sample_messages, sample_model_config):
        """Test that malicious headers cannot be injected through model config."""
        # Attempt to inject malicious headers through model config
        malicious_config = ModelConfig(name="test-model")
        malicious_config.custom_header = "Authorization: Bearer malicious-key"
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            await client.create_chat_completion(
                messages=sample_messages,
                model_config=malicious_config
            )
            
            # Verify that only legitimate headers were used
            call_args = mock_post.call_args
            headers = call_args[1]['headers']
            assert headers['Authorization'] == f'Bearer {client.api_key}'
    
    def test_validate_messages_with_role_case_sensitivity(self, client):
        """Test message validation with different case variations of valid roles."""
        case_variations = [
            ChatMessage(role="USER", content="Test uppercase"),
            ChatMessage(role="Assistant", content="Test titlecase"),
            ChatMessage(role="SYSTEM", content="Test system uppercase")
        ]
        
        # Should reject case variations if implementation is case-sensitive
        for message in case_variations:
            with pytest.raises(ValidationError, match="Invalid message role"):
                client._validate_messages([message])
    
    def test_validate_messages_with_numeric_content(self, client):
        ...
        # Should handle numeric content as strings
        try:
            client._validate_messages(numeric_messages)
        except ValidationError:
            pytest.fail("Numeric content should be valid as strings")
    ...
    def test_validate_model_config_with_string_numeric_values(self, client):
        ...
        # These should be rejected if expecting numeric types
    ...
    @pytest.mark.asyncio
    async def test_api_key_masking_in_logs(self, client, sample_messages, sample_model_config):
        """Test that API keys are properly masked in error logs."""
        with patch('aiohttp.ClientSession.post') as mock_post:
            mock_post.return_value.__aenter__.return_value.status = 401
            mock_post.return_value.__aenter__.return_value.json = AsyncMock(
                return_value={'error': {'message': 'Invalid API key'}}
            ...
            with patch('logging.getLogger') as mock_logger:
                ...
                if mock_log.error.called:
                    log_calls = [call[0][0] for call in mock_log.error.call_args_list]
                    for log_message in log_calls:
                        assert client.api_key not in log_message
    
    @pytest.mark.asyncio
    async def test_request_timeout_with_custom_timeout(self, client_with_custom_timeout, sample_messages, sample_model_config):
        """Test that custom timeout configuration is respected."""
        with patch('aiohttp.ClientSession.post', side_effect=asyncio.TimeoutError()):
            start_time = datetime.now()
            
            with pytest.raises(GenesisAPIError, match="Request timeout"):
                await client_with_custom_timeout.create_chat_completion(
                    messages=sample_messages,
                    model_config=sample_model_config
                )
            
            elapsed = (datetime.now() - start_time).total_seconds()
            # Should timeout around the configured timeout value
            assert elapsed >= 9  # Allow some margin for test execution
    
    @pytest.mark.asyncio
    async def test_zero_retries_configuration(self, client_with_zero_retries, sample_messages, sample_model_config):
        """Test client behavior with zero retry attempts."""
        with patch('aiohttp.ClientSession.post') as mock_post:
            mock_post.return_value.__aenter__.return_value.status = 500
            mock_post.return_value.__aenter__.return_value.json = AsyncMock(
                return_value={'error': {'message': 'Server error'}}
            ...
            with pytest.raises(GenesisAPIError):
                await client_with_zero_retries.create_chat_completion(
                    messages=sample_messages,
                    model_config=sample_model_config
                )
            
            # Should only make one call with zero retries
            assert mock_post.call_count == 1
    
    @pytest.mark.asyncio
    async def test_model_retrieval_with_special_characters(self, client):
        """Test model retrieval with special characters in model ID."""
        special_model_id = "genesis-model-v1.5-beta+test"
        ...
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            model = await client.get_model(special_model_id)
            assert model.id == special_model_id
    
    @pytest.mark.asyncio
    async def test_concurrent_model_listing(self, client):
        """Test concurrent model listing requests."""
        mock_response = {
            'object': 'list',
            'data': [
                {'id': 'model-1', 'object': 'model', 'created': 1677610602},
                {'id': 'model-2', 'object': 'model', 'created': 1677610602}
            ]
        }
...
class TestDataModelValidation:
    """Extended validation tests for data model classes."""
    
    def test_chat_message_content_type_validation(self):
        """Test that ChatMessage validates content type properly."""
        # Test with non-string content
        with pytest.raises((TypeError, ValueError)):
            ChatMessage(role="user", content=123)
        
        with pytest.raises((TypeError, ValueError)):
            ChatMessage(role="user", content=['list', 'content'])
        
        with pytest.raises((TypeError, ValueError)):
            ChatMessage(role="user", content={'dict': 'content'})
    ...
    def test_model_config_parameter_boundaries(self):
        """Test ModelConfig with boundary values for all parameters."""
        config = ModelConfig(name="boundary-test")
        
        # Test frequency_penalty boundaries
        config.frequency_penalty = -2.0  # Minimum
        assert config.frequency_penalty == -2.0
        
        config.frequency_penalty = 2.0  # Maximum
        assert config.frequency_penalty == 2.0
        
        # Test presence_penalty boundaries
        config.presence_penalty = -2.0  # Minimum
        assert config.presence_penalty == -2.0
        
        config.presence_penalty = 2.0  # Maximum
        assert config.presence_penalty == 2.0
    
    def test_model_config_invalid_parameter_types(self):
        """Test ModelConfig with invalid parameter types."""
        config = ModelConfig(name="type-test")
        
        # Test invalid types for numeric parameters
        with pytest.raises((TypeError, ValueError)):
        ...
        with pytest.raises((TypeError, ValueError)):
            config.top_p = "not_a_number"
    
    def test_api_response_with_non_dict_data(self):
        """Test APIResponse with non-dictionary data."""
        # Test with list data
        response = APIResponse(
            status_code=200,
            data=[1, 2, 3],
        ...
        assert response.data == "string response"
    
    def test_chat_completion_with_invalid_usage_data(self):
        """Test ChatCompletion with invalid usage data."""
...
class TestSecurityConsiderations:
    """Tests focused on security aspects of the API client."""
    
    def test_api_key_not_in_string_representation(self, client):
    ...
    def test_sensitive_data_not_in_error_messages(self, client):
        ...
        error = GenesisAPIError(f"Error with key {client.api_key}")
        
        # Error message should not contain the actual API key
        assert client.api_key not in str(error)
    
    @pytest.mark.asyncio
    async def test_ssl_verification_enforcement(self, client):
        """Test that SSL verification is enforced for HTTPS requests."""
        with patch('aiohttp.ClientSession') as mock_session_class:
            mock_session = Mock()
            mock_session_class.return_value = mock_session
            
            # Create new client to trigger session creation
            GenesisAPIClient(api_key='test-key', base_url='https://api.example.com')
            
            # Verify SSL verification is enabled
            call_args = mock_session_class.call_args
            if call_args and 'connector' in call_args[1]:
                # Should not disable SSL verification
                assert call_args[1]['connector'].verify_ssl is not False
    ...
    def test_input_sanitization_in_messages(self, client):
        ...
        for content in potentially_malicious_content:
            message = ChatMessage(role="user", content=content)
            # Should not raise exceptions for malicious content
            try:
                client._validate_messages([message])
            except ValidationError as e:
                # Should fail on content validation, not on injection
...
class TestErrorRecoveryMechanisms:
    ...
    @pytest.mark.asyncio
    async def test_recovery_from_network_interruption(self, client, sample_messages, sample_model_config):
        """Test recovery from network interruption during request."""
        call_count = 0
        
        async def mock_post_with_network_error(*args, **kwargs):
            nonlocal call_count
            call_count += 1
        ...
        with patch('aiohttp.ClientSession.post', side_effect=mock_post_with_network_error):
            with patch('asyncio.sleep'):  # Speed up test
                result = await client.create_chat_completion(
                    messages=sample_messages,
                    model_config=sample_model_config
    ...
    @pytest.mark.asyncio
    async def test_graceful_degradation_on_partial_failures(self, client):
        """Test graceful degradation when some API features are unavailable."""
        # Mock list_models to fail but chat completion to succeed
        with patch('aiohttp.ClientSession.get') as mock_get:
            mock_get.return_value.__aenter__.return_value.status = 503
            mock_get.return_value.__aenter__.return_value.json = AsyncMock(
                return_value={'error': {'message': 'Service unavailable'}}
            )
            
            with pytest.raises(GenesisAPIError):
                await client.list_models()
        
        # Chat completion should still work
        with patch('aiohttp.ClientSession.post') as mock_post:
            mock_post.return_value.__aenter__.return_value.status = 200
            mock_post.return_value.__aenter__.return_value.json = AsyncMock(return_value={
                'id': 'degraded-test',
            ...
            assert result.id == 'degraded-test'
    
    @pytest.mark.asyncio
    async def test_circuit_breaker_behavior_on_repeated_failures(self, client, sample_messages, sample_model_config):
        """Test circuit breaker-like behavior on repeated failures."""
        failure_count = 0
        
        async def mock_post_with_consistent_failure(*args, **kwargs):
            ...
            mock_response.status = 500
            mock_response.json = AsyncMock(
                return_value={'error': {'message': 'Persistent server error'}}
            )
            return mock_response
        
        with patch('aiohttp.ClientSession.post', side_effect=mock_post_with_consistent_failure):
            with patch('asyncio.sleep'):  # Speed up test
...
class TestUtilityFunctionComprehensive:
    """Comprehensive tests for utility functions with edge cases."""
    
    def test_format_timestamp_with_timezone_handling(self):
        """Test timestamp formatting with different timezone scenarios."""
        from app.ai_backend.genesis_api import format_timestamp
        
        # Test various timestamp formats
        test_cases = [
            1677610602,  # Standard Unix timestamp
            1677610602.123,  # Timestamp with microseconds
            0,  # Epoch
    ...
    def test_calculate_token_usage_with_special_characters(self):
        """Test token usage calculation with various special characters."""
        from app.ai_backend.genesis_api import calculate_token_usage
        
        special_messages = [
            ChatMessage(role="user", content="Hello 世界! 🌍"),
            ChatMessage(role="assistant", content="Math: ∑∞≠≤≥±÷×√∆π"),
            ChatMessage(role="user", content="Symbols: ™©®€£¥₹₽"),
            ChatMessage(role="assistant", content="Arrows: ←↑→↓↔↕⇄⇅")
        ]
        
        usage = calculate_token_usage(special_messages)
        assert isinstance(usage, dict)
        assert 'estimated_tokens' in usage
        assert usage['estimated_tokens'] > 0
    
    def test_estimate_tokens_with_markdown_content(self):
        """Test token estimation with markdown formatted content."""
        from app.ai_backend.genesis_api import estimate_tokens
        ...
        **Bold text** and *italic text*
        ...
        assert isinstance(tokens, int)
        assert tokens > 0
        # Should account for markdown formatting
        assert tokens > len(markdown_content.split())
    
    def test_estimate_tokens_with_code_content(self):
        """Test token estimation with code content."""
        from app.ai_backend.genesis_api import estimate_tokens
        ...
        assert tokens >= len(code_content.split())


class TestRobustnessAndStressConditions:
    """Tests for robustness under stress conditions."""
    
    @pytest.mark.asyncio
    async def test_handling_extremely_large_model_list(self, client):
        """Test handling of extremely large model lists."""
        # Create a very large model list
        large_model_list = {
            'object': 'list',
            'data': [
                {'id': f'model-{i}', 'object': 'model', 'created': 1677610602}
                for i in range(10000)  # 10,000 models
            ]
        }
        
        with patch('aiohttp.ClientSession.get') as mock_get:
            mock_get.return_value.__aenter__.return_value.json = AsyncMock(return_value=large_model_list)
            mock_get.return_value.__aenter__.return_value.status = 200
            
            models = await client.list_models()
            assert len(models) == 10000
    
    @pytest.mark.asyncio
    async def test_handling_deeply_nested_response_data(self, client, sample_messages, sample_model_config):
        """Test handling of deeply nested response structures."""
        deeply_nested_response = {
            'id': 'nested-test',
            'object': 'chat.completion',
            'created': 1677610602,
            'model': 'test-model',
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            mock_post.return_value.__aenter__.return_value.json = AsyncMock(return_value=deeply_nested_response)
            mock_post.return_value.__aenter__.return_value.status = 200
            ...
            assert result.id == 'nested-test'
    
    @pytest.mark.asyncio
    async def test_memory_efficiency_with_streaming(self, client, sample_messages, sample_model_config):
        """Test memory efficiency during long streaming sessions."""
        # Simulate a very long streaming response
        chunk_count = 5000
        
        async def long_mock_stream():
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            async for chunk in client.create_chat_completion_stream(
                ...
                processed_chunks += 1
                # Verify memory is not accumulating unnecessarily
                if processed_chunks % 1000 == 0:
                    # This is just a placeholder - in real tests you'd check memory usage
                    assert processed_chunks > 0
            
            assert processed_chunks == chunk_count + 1  # +1 for finish chunk
