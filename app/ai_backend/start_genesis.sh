...
# print_status prints an informational message to stdout prefixed with a blue [INFO] label.
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success prints the given message prefixed with a green [SUCCESS] label to indicate a successful operation.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning prints a message to stdout prefixed with a yellow [WARNING] label to indicate a warning condition.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error prints a message to stdout prefixed with a red [ERROR] label to indicate an error.
print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

# print_genesis prints a message prefixed with a purple [GENESIS] label for Genesis Layer notifications.
print_genesis() {
