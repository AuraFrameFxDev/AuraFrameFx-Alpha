...
# print_status prints an informational message to stdout with a blue [INFO] prefix.
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success prints a message with a green [SUCCESS] prefix to indicate a successful operation.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning prints a warning message with a yellow [WARNING] prefix.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error prints an error message with a red [ERROR] prefix.
print_error() {
