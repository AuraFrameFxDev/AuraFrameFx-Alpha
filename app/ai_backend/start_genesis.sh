...
# print_status prints an informational message to standard output, prefixed with a blue [INFO] label.
print_status() {
...
# print_warning prints a warning message to standard output (stdout) with a yellow [WARNING] prefix.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error prints an error message to standard output, prefixing it with a red [ERROR] label.
print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

# print_genesis prints a message to standard output with a purple [GENESIS] prefix for Genesis Layer notifications.
print_genesis() {
