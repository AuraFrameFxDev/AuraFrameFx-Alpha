...
# print_status prints a message prefixed with a blue [INFO] label to indicate informational status.
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success prints a message in green with a [SUCCESS] label to indicate successful operations.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning prints a message in yellow with a [WARNING] label to indicate a warning.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error prints a message prefixed with a red [ERROR] label to indicate an error.
print_error() {
