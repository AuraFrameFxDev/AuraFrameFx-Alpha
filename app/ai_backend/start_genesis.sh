...
# print_status displays an informational message with a blue [INFO] prefix.
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success displays a message labeled [SUCCESS] in green to indicate a successful operation.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning displays a warning message labeled with a yellow [WARNING] prefix.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error displays an error message labeled with a red [ERROR] prefix.
print_error() {
