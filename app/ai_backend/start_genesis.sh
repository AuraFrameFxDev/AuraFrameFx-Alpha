...
# print_status prints an informational message labeled with a blue [INFO] tag.
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success prints a message labeled [SUCCESS] in green to indicate a successful operation.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning prints a message labeled with a yellow [WARNING] tag to indicate a warning.
print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

# print_error prints a message labeled with a red [ERROR] tag to indicate an error.
print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

# print_genesis displays a message labeled with a purple [GENESIS] tag for Genesis Layer notifications.
print_genesis() {
