...
def mock_base_url():
    """
    Provides a mock base URL string representing the Genesis API endpoint for use in integration tests.
...
def sample_api_response():
    ...
    The returned dictionary includes fields such as response ID, object type, creation timestamp, model name, assistant message, finish reason, and token usage statistics. Useful for tests that require a realistic Genesis API response structure.
...
def mock_environment():
    ...
    This autouse pytest fixture ensures that `GENESIS_API_KEY` and `GENESIS_BASE_URL` are set to test values for the duration of each test, then cleans them up to maintain test isolation.
    """
