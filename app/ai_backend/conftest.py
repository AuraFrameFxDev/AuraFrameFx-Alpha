...
def sample_api_response():
    ...
    Returns:
        dict: A simulated API response including metadata, an assistant message, finish reason, and token usage details.
    """
    return {
        "id": "test_response_id",
...
def sample_error_response():
    """
    Return a dictionary simulating a typical error response from the Genesis API.
    
    The response includes error details such as type, message, parameter, and code.
    
    Returns:
        dict: A dictionary containing error information representative of a Genesis API error response.
    """
    return {
        "error": {
...
def mock_environment():
    """
    Automatically sets and cleans up test environment variables for Genesis API client tests.
