...
def sample_api_response():
    ...
    The returned dictionary contains fields such as response ID, object type, creation timestamp, model name, a list of choices with an assistant message and finish reason, and token usage statistics. Useful for tests that require a realistic Genesis API response structure.
...
def sample_error_response():
    ...
    The returned dictionary contains an `error` object with fields for error type, message, parameter, and code, enabling tests to verify error handling logic for Genesis API integration.
...
def mock_environment():
    ...
    This autouse pytest fixture ensures that `GENESIS_API_KEY` and `GENESIS_BASE_URL` are set to test values for the duration of each test, maintaining isolation between test runs.
    """
