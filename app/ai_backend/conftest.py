...
def mock_api_key():
    """
    Provide a fixed mock API key string for Genesis API client tests.
...
def sample_api_response():
    ...
    Returns:
        dict: Contains response metadata, assistant message, finish reason, and token usage statistics.
    """
    return {
        "id": "test_response_id",
...
def sample_error_response():
    ...
    The dictionary contains an "error" object with fields for error type, message, parameter, and code.
    """
    return {
        "error": {
...
def mock_environment():
    ...
    This autouse fixture assigns test values to `GENESIS_API_KEY` and `GENESIS_BASE_URL` before each test and deletes them after the test completes to maintain isolation between tests.
    """
