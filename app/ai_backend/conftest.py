...
def mock_base_url():
    """
    Provides a mock base URL for the Genesis API endpoint used in integration tests.
...
def sample_api_response():
    ...
    The returned dictionary contains metadata, a list of choices with an assistant message and finish reason, and token usage statistics. Useful for tests that require a realistic Genesis API response structure.
...
def mock_environment():
    """
    Sets Genesis API environment variables to test values before each test and removes them afterward to maintain test isolation.
    
    This autouse pytest fixture ensures that `GENESIS_API_KEY` and `GENESIS_BASE_URL` are consistently set for each test run and are cleaned up after the test completes.
    """
