...
def sample_api_response():
    ...
    The dictionary contains metadata, assistant message choices, finish reason, and token usage statistics for use in integration tests.
...
def sample_error_response():
    """
    Provides a mock dictionary representing an error response from the Genesis API for testing purposes.
    
    Returns:
        dict: Contains an `error` object with fields for type, message, parameter, and code.
    """
    return {
        "error": {
...
def mock_environment():
    """
    Configures and cleans up Genesis API environment variables before and after each test to ensure test isolation.
    
    This autouse pytest fixture sets `GENESIS_API_KEY` and `GENESIS_BASE_URL` to test values for the duration of each test and removes them afterward to prevent environment variable leakage between tests.
    """
