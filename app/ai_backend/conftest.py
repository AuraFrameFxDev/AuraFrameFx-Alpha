...
def sample_api_response():
    ...
    The returned dictionary includes response metadata, an assistant message, a finish reason, and token usage statistics. Useful for testing code that interacts with the Genesis API.
...
def sample_error_response():
    ...
    The returned dictionary mimics the structure of an error response from the Genesis API, including fields for error type, message, parameter, and code. Useful for testing error handling in integration tests.
    
    Returns:
        dict: Mock error response containing error details.
    """
    return {
        "error": {
