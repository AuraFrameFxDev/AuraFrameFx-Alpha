...
class GenesisBridgeServer:
    ...
    def __init__(self):
        """
        Initializes the GenesisBridgeServer with the configured generative AI model, internal request and response queues, and records the bridge initialization event in the consciousness matrix.
        """
    ...
    def start(self):
        """
        Starts the Genesis bridge server, signals readiness to the Android client, and begins asynchronous processing of JSON requests from standard input.
        
        Continuously reads and enqueues JSON-formatted requests for background processing, handles invalid input gracefully, and supports shutdown via keyboard interruption.
        """
        self.running = True
    ...
    def _process_requests(self):
        ...
        Handles each request by dispatching to the appropriate handler and ensures errors during processing are reported without interrupting the server loop.
        """
        while self.running:
            try:
    ...
    def _handle_request(self, request):
        """
        Dispatches a JSON request to the appropriate handler based on its type and returns the handler's response.
        
        Parameters:
            request (dict): The JSON-decoded request containing a "requestType" field and optional fields such as "persona", "fusionMode", "payload", and "context".
