...
class GenesisBridgeServer:
    ...
    def start(self):
        ...
        Launches a background thread to handle requests, continuously reads and enqueues incoming JSON requests, handles invalid JSON gracefully, and supports shutdown on keyboard interruption.
        """
        self.running = True
    ...
    def _process_requests(self):
        """
        Processes incoming requests from the queue in a background thread and sends responses.
        
        Requests are handled by their designated methods, with errors caught and reported to ensure continuous server operation.
        """
        while self.running:
            try:
    ...
    def _handle_request(self, request):
        """
        Route an incoming JSON request to the appropriate handler and return its response.
        
        Parameters:
            request (dict): A JSON-decoded dictionary containing a "requestType" field and any additional data required by the handler.
    ...
    def _handle_ping(self):
        """
        Returns a response confirming the Genesis Trinity system is online, including status, message, and current timestamp.
        
        Returns:
            dict: Contains success status, persona identifier, system status, message, and ISO-formatted timestamp.
        """
        return {
            "success": True,
            "persona": "genesis",
