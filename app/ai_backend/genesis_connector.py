...
class GenesisBridgeServer:
    ...
    def _process_requests(self):
        """
        Continuously processes queued requests in a background thread, dispatching each to the appropriate handler and sending responses to the client.
        
        Exceptions encountered during processing are caught and reported to maintain server responsiveness.
        """
        while self.running:
            try:
    ...
    def _handle_request(self, request):
        """
        Routes a JSON-decoded request to the appropriate handler based on the "requestType" field and returns the handler's response.
        
        Parameters:
            request (dict): A dictionary containing a "requestType" key and any additional data required by the handler.
    ...
    def _handle_ping(self):
        """
        Return a response indicating the Genesis Trinity system is online, including status, message, and the current timestamp.
