...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, sets the `is_running` attribute to False. Any exceptions during shutdown are logged.
        """
        try:
...
def health_check():
    """
    Return the Genesis API health status as a JSON response.
    
    The response includes:
    - "status": "healthy" if the Genesis Layer backend is running, otherwise "unhealthy".
...
def chat_with_genesis():
    ...
    Expects a JSON payload with required fields `message` and `user_id`, and an optional `context` object. Returns HTTP 400 if the request is not JSON or if required fields are missing, and HTTP 500 for internal server errors.
...
def get_status():
    ...
    Returns:
        A JSON response containing the Genesis Layer's status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def reset_session():
    ...
    Returns:
        Response: JSON containing the reset status, message, and timestamp on success. Returns HTTP 500 with an error message if the reset operation fails.
    """
    try:
...
def internal_error(error):
    """
    Handles HTTP 500 Internal Server Error responses by returning a generic JSON error message.
...
def initialize_app():
    """
    Initializes the Genesis Layer backend asynchronously before handling the first client request.
    """
