...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        Sets `is_running` to False after successful shutdown. Logs any errors encountered during the shutdown process.
        """
        try:
...
def health_check():
    """
    Return the Genesis API health status as a JSON object with current server time and backend uptime.
...
def chat_with_genesis():
    """
    Processes user chat messages sent to the /genesis/chat endpoint by forwarding the message, user ID, and optional context to the Genesis backend and returning its response as JSON.
    ...
    Returns:
        ...
        On error: JSON error message with HTTP 400 for invalid input or HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_genesis_profile():
    ...
    Returns:
        JSON containing the Genesis profile fields on success, or a JSON error message with HTTP 500 status if retrieval fails.
    """
    try:
        profile_data = {
...
def reset_session():
    ...
    Returns:
        Response: A JSON response with `status`, `message`, and `timestamp` fields if the reset is successful, or an error message with HTTP 500 status if the reset fails.
    """
    try:
