...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance, setting the backend state to inactive and clearing the start time.
        """
    ...
    async def shutdown(self):
        ...
        After shutdown, sets the backend running state to False. Any exceptions during shutdown are logged.
        """
        try:
...
def get_status():
    ...
    Returns:
        Response: A Flask JSON response containing the Genesis Layer's status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def internal_error(error):
    ...
    Returns:
        tuple: JSON response containing an error description and message, with HTTP status code 500.
    """
    return jsonify({
        "error": "Internal server error",
