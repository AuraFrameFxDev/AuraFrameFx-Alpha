...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with the backend set as not running and no recorded start time.
        """
    ...
    async def startup(self):
        """
        Asynchronously initializes the Genesis backend and sets the running state.
...
def chat_with_genesis():
    """
    Processes a chat request by forwarding the user's message and optional context to the Genesis backend and returning its response as JSON.
    
    Expects a JSON payload containing "message" (string) and "user_id" (string); "context" is optional. Returns HTTP 400 for missing or invalid fields, and HTTP 500 for internal server errors.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        JSON response with the backend's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    ...
    The response includes identity, personality, capabilities, values, and evolution stage. If profile retrieval fails, returns an HTTP 500 error with an error message.
    """
    try:
        profile_data = {
...
def evaluate_ethics():
    """
    Handles POST requests to evaluate the ethical implications of a specified action using the Genesis backend.
...
def reset_session():
    ...
    Returns:
        Response: JSON object indicating reset status, message, and timestamp on success; returns a JSON error message with HTTP 500 status if the reset fails.
    """
    try:
...
def not_found(error):
    """
    Return a JSON error message and HTTP 404 status code when an unknown API endpoint is requested.
...
def internal_error(error):
    """
    Return a generic JSON error response with HTTP 500 status for unexpected server errors.
