...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, sets the `is_running` attribute to False. Any exceptions during shutdown are caught and logged.
        """
        try:
...
def chat_with_genesis():
    """
    Process a chat message from the user by forwarding it to the Genesis backend and returning the backend's response.
    ...
    Returns:
        On success, a JSON response from the Genesis backend.
        On error, a JSON error message with HTTP 400 for invalid input or HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        Response: Flask JSON response containing the Genesis Layer's status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def reset_session():
    ...
    Returns:
        Response: A JSON response with `status`, `message`, and `timestamp` fields if the reset is successful. Returns a JSON error message with HTTP 500 status if the reset fails.
    """
    try:
