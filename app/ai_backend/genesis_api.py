...
def run_async(coro):
    ...
    Intended for use in synchronous contexts to bridge calls to asynchronous backend functions.
...
def chat_with_genesis():
    """
    Handles a chat request by sending the user's message and optional context to the Genesis backend and returning its response as JSON.
    
    Expects a JSON payload with "message" (string) and "user_id" (string); "context" is optional. Responds with HTTP 400 for missing or invalid fields, and HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_status():
    """
    Retrieve and return the current status of the Genesis backend as a JSON response.
    
    Returns:
        A JSON object containing the backend's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def reset_session():
    ...
    Returns:
        Response: JSON object with status, message, and timestamp on success; JSON error message with HTTP 500 status on failure.
    """
    try:
...
def internal_error(error):
    """
    Return a generic JSON response with HTTP 500 status for unexpected internal server errors.
