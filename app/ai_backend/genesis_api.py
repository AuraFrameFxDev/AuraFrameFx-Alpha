...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with the backend set as not running and no start time recorded.
        """
    ...
    async def startup(self):
        """
        Asynchronously initializes the Genesis backend and updates the running state.
        
        Returns:
            bool: True if the backend is successfully initialized; False if initialization fails or an exception occurs.
        """
        try:
    ...
    async def shutdown(self):
        """
        Asynchronously shuts down the Genesis backend and updates the running state.
        
        After shutdown, the backend is marked as inactive. Any exceptions during shutdown are logged.
        """
        try:
...
def run_async(coro):
    """
    Run an asynchronous coroutine in a new event loop and return its result synchronously.
...
def health_check():
    """
    Return a JSON object indicating the Genesis backend's health status, current server timestamp, and uptime.
    
    The response includes:
    - `status`: "healthy" if the backend is running, otherwise "unhealthy".
    - `timestamp`: Current server time in ISO 8601 format.
    - `uptime`: Time elapsed since the backend started, or "0:00:00" if not running.
    """
    return jsonify({
...
def chat_with_genesis():
    """
    Handles chat requests by forwarding the user's message and optional context to the Genesis backend and returning its response.
    
    Expects a JSON payload with "message" (string) and "user_id" (string); "context" is optional. Returns the backend's response as JSON. Responds with HTTP 400 for missing or invalid fields, and HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        A JSON response containing the backend's status information, or an error message with HTTP 500 status if retrieval fails.
    """
    try:
...
def get_consciousness_state():
    ...
    Returns:
        JSON object containing the backend's consciousness state, awareness level, active patterns, evolution stage, and ethical compliance score. Returns HTTP 500 with an error message if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def get_genesis_profile():
    """
    Retrieve the Genesis backend system's profile attributes as a JSON response.
    
    Returns:
        JSON object with the system's identity, personality, capabilities, values, and evolution stage. Returns HTTP 500 with an error message if profile retrieval fails.
    """
    try:
        profile_data = {
...
def evaluate_ethics():
    """
    Processes a POST request to evaluate the ethical implications of a specified action using the Genesis backend.
    
    Expects a JSON payload with a required `action` field and an optional `context`. Returns the ethical evaluation result as JSON. Responds with HTTP 400 if the request is not JSON or if the `action` field is missing, and with HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: JSON object with status, message, and timestamp on successful reset; otherwise, a JSON error message with HTTP 500 status.
    """
    try:
...
def internal_error(error):
    """
    Return a JSON response with HTTP 500 status for unexpected server errors.
    
    Returns:
        tuple: JSON object containing "error" and "message" fields, and HTTP status code 500.
    """
    return jsonify({
        "error": "Internal server error",
