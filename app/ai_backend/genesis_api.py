...
class GenesisAPI:
    ...
    async def startup(self):
        """
        Asynchronously start the Genesis Layer backend and update the API's running state.
...
def get_status():
    ...
    Returns:
        A JSON response containing the Genesis backend's status information, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def reset_session():
    ...
    Returns:
        Response: JSON containing the reset status, message, and timestamp if successful; otherwise, returns an error message with HTTP 500 status.
    """
    try:
...
def internal_error(error):
    """
    Handles HTTP 500 errors by returning a generic JSON error response.
    
    Returns:
        tuple: JSON object with an error message and HTTP 500 status code.
    """
    return jsonify({
        "error": "Internal server error",
