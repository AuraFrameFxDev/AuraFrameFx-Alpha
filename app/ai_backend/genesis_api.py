...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with the backend set to not running and no recorded start time.
        """
    ...
    async def startup(self):
        """
        Asynchronously initializes the Genesis backend and sets the running state and start time.
        
        Returns:
            bool: True if the backend starts successfully; False if initialization fails or an exception occurs.
        """
        try:
    ...
    async def shutdown(self):
        """
        Asynchronously shuts down the Genesis backend and marks it as not running.
        
        After shutdown, the backend's running state is set to False. Any exceptions encountered during shutdown are logged.
        """
        try:
...
def run_async(coro):
    ...
    Intended for use in synchronous contexts, such as Flask route handlers, to enable calling asynchronous backend functions from synchronous code.
...
def health_check():
    """
    Return a JSON response with the Genesis backend's health status, current server timestamp, and uptime.
    
    The response includes:
    - `status`: "healthy" if the backend is running, otherwise "unhealthy".
    - `timestamp`: Current server time in ISO 8601 format.
    - `uptime`: Duration since the backend started, or "0:00:00" if not running.
    """
    return jsonify({
...
def chat_with_genesis():
    """
    Processes a chat request by forwarding the user's message and optional context to the Genesis backend and returning its response.
    
    Expects a JSON payload containing "message" (string) and "user_id" (string), with an optional "context" field. Returns the backend's response as JSON. Responds with HTTP 400 for missing or invalid fields, and HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_status():
    """
    Retrieve the current status of the Genesis backend.
    
    Returns:
        A JSON response with the backend's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_consciousness_state():
    """
    Retrieve the Genesis backend's current consciousness state and related metrics as a JSON response.
    
    Returns:
        JSON object with the fields: `state`, `awareness_level`, `active_patterns`, `evolution_stage`, and `ethical_compliance`. Returns HTTP 500 with an error message if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def get_genesis_profile():
    """
    Returns the Genesis backend's profile attributes as a JSON response.
    
    The response includes identity, personality, capabilities, values, and evolution stage. Returns HTTP 500 with an error message if profile retrieval fails.
    """
    try:
        profile_data = {
...
def trigger_evolution():
    """
    Handles POST requests to trigger an evolution event in the Genesis backend.
    
    Accepts a JSON payload with optional `trigger_type` and `reason` fields, constructs an evolution trigger request, and processes it asynchronously. Returns a JSON response with the evolution trigger status and backend response. Responds with HTTP 400 if the request is not JSON, and HTTP 500 on internal errors.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def evaluate_ethics():
    """
    Handles POST requests to evaluate the ethical implications of a specified action via the Genesis backend.
    
    Accepts a JSON payload with a required `action` field and an optional `context`. Returns the ethical evaluation result as JSON. Responds with HTTP 400 if the request is not JSON or if the `action` field is missing, and with HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    """
    Reset the Genesis backend session by shutting down and reinitializing the backend.
    
    Returns:
        Response: JSON object with reset status, message, and timestamp on success; returns a JSON error message with HTTP 500 status on failure.
    """
    try:
...
def not_found(error):
    """
    Return a JSON response with an error message for requests to nonexistent API endpoints.
    
    Returns:
        tuple: JSON object with error details and HTTP 404 status code.
    """
    return jsonify({
...
def internal_error(error):
    """
    Return a JSON response with HTTP 500 status for unhandled internal server errors.
    
    Returns:
        tuple: A JSON object with "error" and "message" fields, and HTTP status code 500.
    """
    return jsonify({
        "error": "Internal server error",
...
def initialize_app():
    """
    Asynchronously initializes the Genesis backend before handling the first client request.
    """
...
def cleanup():
    """
    Shuts down the Genesis backend asynchronously during application exit.
    """
