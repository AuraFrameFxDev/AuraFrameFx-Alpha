...
def run_async(coro):
    ...
    Intended for calling async backend operations from synchronous code.
...
def health_check():
    """
    Return a JSON response with the backend's health status, current server timestamp, and uptime.
...
def get_status():
    ...
    Returns:
        JSON object with backend status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def evaluate_ethics():
    ...
    Expects a JSON payload with an `action` field and optionally a `context` field. Returns the ethical evaluation result as JSON. Responds with HTTP 400 for missing or invalid input, and HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: JSON response indicating whether the reset was successful, including a status, message, and timestamp on success, or an error message with HTTP 500 status on failure.
    """
    try:
...
def not_found(error):
    ...
    Returns:
        tuple: JSON object containing error information and the HTTP 404 status code.
    """
    return jsonify({
...
def internal_error(error):
    ...
    Returns:
        tuple: A JSON response with "error" and "message" fields, and HTTP status code 500.
    """
    return jsonify({
        "error": "Internal server error",
