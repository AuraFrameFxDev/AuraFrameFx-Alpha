...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is set to a non-running state. Any exceptions encountered during the shutdown process are logged.
        """
        try:
...
def health_check():
    ...
    - `uptime`: Duration since the backend started, or "0:00:00" if inactive.
    """
    return jsonify({
...
def chat_with_genesis():
    """
    Processes a chat request by sending the user's message and optional context to the Genesis backend and returning its response.
    
    Expects a JSON payload with "message" (string) and "user_id" (string), and optionally "context". Returns the backend's response as JSON. Responds with HTTP 400 if the request is not JSON or required fields are missing, and HTTP 500 on internal errors.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        A JSON object with the backend's status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    """
    Returns the Genesis system's profile attributes as a JSON object.
...
def evaluate_ethics():
    ...
    Accepts a JSON payload with a required `action` field and an optional `context`. Returns the ethical evaluation result as a JSON object. Responds with HTTP 400 if the request is not JSON or if the `action` field is missing, and with HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def internal_error(error):
    """
    Handle unhandled server errors by returning a generic JSON error response with HTTP 500 status code.
