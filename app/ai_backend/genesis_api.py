...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with inactive running state and no start time set.
        """
    ...
    async def shutdown(self):
        ...
        Sets the `is_running` attribute to False after a successful shutdown. Errors during shutdown are logged.
        """
        try:
...
def run_async(coro):
    ...
    Intended for use within Flask routes to bridge async backend calls with synchronous HTTP handlers.
    """
...
def health_check():
    """
    Return the current health status, server timestamp, and uptime of the Genesis API.
    
    The response JSON includes the API's health ("healthy" if the Genesis Layer is running, otherwise "unhealthy"), the current server time in ISO 8601 format, and the uptime since the Genesis Layer was started.
    """
    return jsonify({
...
def chat_with_genesis():
    """
    Processes chat requests by validating input and forwarding user messages to the Genesis backend for response.
    
    Expects a JSON payload with a `message` and `user_id`, and optionally a `context` object. Returns the Genesis backend's response as JSON, or an error message with appropriate HTTP status if validation fails or an internal error occurs.
    """
    try:
        # Validate request
...
def get_status():
    """
    Retrieve the current status information of the Genesis Layer.
    
    Returns:
        A JSON response with comprehensive Genesis Layer status details, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    """
    Returns the Genesis system's personality profile and identity details as a JSON response.
...
def reset_session():
    ...
    Returns:
        Response: A JSON response indicating the outcome of the reset operation, including status, message, and timestamp on success, or an error message with HTTP 500 on failure.
    """
    try:
...
def internal_error(error):
    """
    Handle HTTP 500 Internal Server Error responses by returning a JSON object with an error message and status code 500.
    """
    return jsonify({
        "error": "Internal server error",
