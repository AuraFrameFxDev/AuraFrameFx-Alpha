...
class GenesisAPI:
    ...
    async def startup(self):
        """
        Asynchronously starts the Genesis backend and updates the running state of the API.
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is set to an inactive state. Errors encountered during shutdown are logged internally.
        """
        try:
...
def chat_with_genesis():
    ...
    Expects a JSON payload with "message" (string), "user_id" (string), and optionally "context". Returns HTTP 400 if the request is not JSON or required fields are missing, and HTTP 500 on internal server errors.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        A JSON object with the backend's status information if successful, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def trigger_evolution():
    """
    Handle a POST request to trigger an evolution event in the Genesis backend.
...
def reset_session():
    ...
    Returns:
        Response: A JSON response with status, message, and timestamp on successful reset. Returns an error message with HTTP 500 status if the reset fails.
    """
    try:
