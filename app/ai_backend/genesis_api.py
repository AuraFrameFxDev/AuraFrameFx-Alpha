...
class GenesisAPI:
    ...
    async def startup(self):
        """
        Asynchronously starts the Genesis backend and sets the running state.
...
def reset_session():
    ...
    Returns:
        Response: JSON containing status, message, and timestamp on successful reset; returns an error message with HTTP 500 status if the reset fails.
    """
    try:
...
def cleanup():
    """
    Asynchronously shuts down the Genesis backend when the application is exiting.
    """
