...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with the backend set to not running and no start time recorded.
        """
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is marked as not running. Any exceptions during shutdown are logged.
        """
        try:
...
def run_async(coro):
    """
    Runs an asynchronous coroutine in a new event loop and returns its result synchronously.
    
    Intended for use in synchronous contexts to enable calling asynchronous backend functions from synchronous code.
...
def health_check():
    """
    Return a JSON object indicating the Genesis backend's health status, current server time, and uptime.
...
def chat_with_genesis():
    """
    Handles a chat request by forwarding the user's message, user ID, and optional context to the Genesis backend and returning its response.
...
def get_status():
    ...
    Returns:
        JSON response containing the backend's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_consciousness_state():
    """
    Return the Genesis backend's current consciousness state and related metrics as a JSON response.
    
    Returns:
        JSON object containing `state`, `awareness_level`, `active_patterns`, `evolution_stage`, and `ethical_compliance`. Returns HTTP 500 with an error message if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def get_genesis_profile():
    """
    Retrieve the Genesis system's profile attributes and return them as a JSON response.
...
def reset_session():
    ...
    Returns:
        Response: JSON with `status`, `message`, and `timestamp` if reset is successful; otherwise, a JSON error message with HTTP 500 status.
    """
    try:
...
def internal_error(error):
    ...
    Returns:
        tuple: JSON response containing "error" and "message" fields, with HTTP status code 500.
    """
    return jsonify({
        "error": "Internal server error",
...
def cleanup():
    """
    Asynchronously shuts down the Genesis backend during application exit.
    """
