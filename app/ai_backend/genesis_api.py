...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is no longer considered running. Any errors encountered during shutdown are logged internally.
        """
        try:
...
def chat_with_genesis():
    ...
    Expects a JSON payload with "message" (string) and "user_id" (string); "context" is optional. Returns the backend's response as JSON. Responds with HTTP 400 if the request is not JSON or required fields are missing, and HTTP 500 on internal errors.
    """
    try:
        # Validate request
...
def reset_session():
    ...
    Returns:
        Response: JSON object indicating reset status, message, and timestamp on success, or an error message with HTTP 500 status on failure.
    """
    try:
