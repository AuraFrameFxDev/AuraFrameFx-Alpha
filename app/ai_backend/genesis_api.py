...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, sets the `is_running` attribute to False. Exceptions during shutdown are logged.
        """
        try:
...
def health_check():
    """
    Return the Genesis API health status as a JSON object, including backend running state, current server timestamp, and uptime.
...
def get_status():
    ...
    Returns:
        JSON object with the Genesis Layer's status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def get_consciousness_state():
    ...
    Returns:
        JSON response containing consciousness state metrics, or an error message with HTTP 500 status if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def reset_session():
    ...
    Returns:
        Response: JSON indicating whether the reset was successful, including status, message, and timestamp on success, or an error message with HTTP 500 if the reset fails.
    """
    try:
