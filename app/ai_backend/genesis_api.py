...
class GenesisAPI:
    ...
    async def shutdown(self):
        """
        Asynchronously shuts down the Genesis backend and marks the API as not running.
        
        Any exceptions encountered during shutdown are logged.
        """
        try:
...
def run_async(coro):
    ...
    Intended for use in synchronous Flask route handlers to enable calling asynchronous backend operations.
    
    Parameters:
        coro: The coroutine object to execute.
    
    Returns:
        The result returned by the coroutine upon completion.
    """
...
def chat_with_genesis():
    """
    Processes a chat request by forwarding the user's message, user ID, and optional context to the Genesis backend and returning the backend's JSON response.
    ...
    Returns:
        Flask Response: JSON response from the Genesis backend on success, or an error message with the appropriate HTTP status code on failure.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        A JSON response containing the Genesis backend's status information with HTTP 200 on success, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    ...
    Returns:
        JSON containing the system's identity, personality, capabilities, values, and evolution stage.
        Returns an error message with HTTP 500 status if profile retrieval fails.
    """
    try:
        profile_data = {
...
def trigger_evolution():
    """
    Trigger an evolution event in the Genesis backend using a specified trigger type and reason.
    
    Accepts a JSON payload with `trigger_type` and `reason`, constructs an evolution trigger request, and processes it asynchronously. Returns a JSON response with the trigger status and backend response on success.
    
    Returns:
        JSON response with:
            - `status`: Indicates evolution was triggered.
            - `response`: Backend response to the evolution trigger.
    
    HTTP Status Codes:
        200: Evolution event triggered successfully.
        400: Request is not JSON.
        500: Internal server error during evolution trigger.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def evaluate_ethics():
    ...
    Accepts a JSON payload with a required `action` field and an optional `context`. Returns the ethical evaluation result as JSON. Responds with HTTP 400 if the request is not JSON or if `action` is missing, and with HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: JSON object with `status`, `message`, and `timestamp` if the reset is successful. Returns HTTP 500 with an error message if the reset fails.
    """
    try:
...
def initialize_app():
    """
    Initialize the Genesis backend asynchronously before handling the first incoming client request.
    """
