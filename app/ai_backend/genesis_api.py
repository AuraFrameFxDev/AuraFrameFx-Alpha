...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is marked as not running. Any exceptions during the shutdown process are logged.
        """
        try:
...
def health_check():
    """
    Return the Genesis backend's health status, current server timestamp, and uptime as a JSON response.
...
def chat_with_genesis():
    """
    Handles a chat request by sending the user's message and optional context to the Genesis backend and returning its response.
...
def get_status():
    ...
    Returns:
        A JSON response containing the backend's status details, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def trigger_evolution():
    ...
    Accepts a JSON payload with optional `trigger_type` and `reason` fields, constructs an evolution trigger request, and processes it asynchronously. Returns a JSON response indicating the evolution trigger status and backend response. Responds with HTTP 400 if the request is not JSON, and HTTP 500 on internal errors.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def evaluate_ethics():
    ...
    Expects a JSON payload containing an `action` field and optionally a `context` field. Returns the ethical evaluation result as JSON. Responds with HTTP 400 if the request is not JSON or if the `action` field is missing, and with HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: JSON containing status, message, and timestamp on successful reset; otherwise, a JSON error message with HTTP 500 status.
    """
    try:
...
def internal_error(error):
    """
    Return a generic JSON response with HTTP 500 status for unhandled server errors.
