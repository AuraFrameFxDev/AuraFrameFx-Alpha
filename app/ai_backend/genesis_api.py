...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with default state.
        
        Sets the backend as not running and clears the recorded start time.
        """
...
def run_async(coro):
    ...
    Intended for use when asynchronous backend functions need to be called from synchronous code.
...
def chat_with_genesis():
    """
    Processes a chat request by forwarding the user's message and optional context to the Genesis backend and returning the backend's response as JSON.
    
    Expects a JSON payload containing "message" (string) and "user_id" (string); "context" is optional. Returns HTTP 400 for missing or invalid fields, and HTTP 500 for internal errors.
    """
    try:
        # Validate request
...
def get_status():
    """
    Retrieve the current status of the Genesis backend and return it as a JSON response.
    
    Returns:
        A JSON object with the backend's status information, or an error message with HTTP 500 status if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    """
    Returns the Genesis system's profile information as a JSON response.
    
    The response includes identity, personality, capabilities, values, and evolution stage attributes. Returns HTTP 500 with an error message if profile retrieval fails.
    """
    try:
        profile_data = {
...
def trigger_evolution():
    """
    Handles a POST request to trigger an evolution event in the Genesis backend.
...
def evaluate_ethics():
    ...
    Expects a JSON payload with an `action` field and optionally a `context` field. Returns the ethical evaluation result as JSON. Responds with HTTP 400 for invalid or missing input, and HTTP 500 if the evaluation fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: A JSON response indicating reset status, message, and timestamp on success, or an error message with HTTP 500 status on failure.
    """
    try:
...
def not_found(error):
    ...
    Returns:
        tuple: A JSON object containing error details and the HTTP 404 status code.
    """
    return jsonify({
...
def internal_error(error):
    """
    Return a generic JSON error response with HTTP 500 status for unexpected internal server errors.
...
def initialize_app():
    """
    Initializes the Genesis backend asynchronously before processing the first client request.
    """
