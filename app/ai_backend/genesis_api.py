...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend's running state is set to False. Any exceptions encountered during the shutdown process are logged.
        """
        try:
...
def run_async(coro):
    ...
    Intended for use in synchronous contexts, such as Flask routes, to enable calling asynchronous backend functions.
...
def chat_with_genesis():
    """
    Processes a chat request by sending the user's message, user ID, and optional context to the Genesis backend and returning the backend's response as JSON.
...
def get_status():
    """
    Return the current operational status of the Genesis backend as a JSON response.
    
    Returns:
        A JSON object containing the backend's status information, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    ...
    The response includes identity, personality, capabilities, values, and evolution stage. Returns an HTTP 500 error with a message if profile retrieval fails.
    """
    try:
        profile_data = {
...
def evaluate_ethics():
    """
    Handles POST requests to evaluate the ethical implications of a given action via the Genesis backend.
...
def reset_session():
    ...
    Returns:
        Response: JSON indicating reset status, message, and timestamp if successful, or an error message with HTTP 500 status on failure.
    """
    try:
