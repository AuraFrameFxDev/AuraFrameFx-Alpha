...
def get_consciousness_state():
    ...
    The response includes `state`, `awareness_level`, `active_patterns`, `evolution_stage`, and `ethical_compliance`. Returns HTTP 500 with an error message if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def reset_session():
    ...
    Returns:
        Response: A JSON object containing the reset status, message, and timestamp if successful; otherwise, a JSON error message with HTTP 500 status.
    """
    try:
...
def not_found(error):
    ...
    Returns:
        tuple: JSON object containing `error` and `message` fields, and HTTP 404 status code.
    """
    return jsonify({
