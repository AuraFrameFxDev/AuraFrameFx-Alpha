...
class GenesisAPI:
    ...
    async def startup(self):
        ...
        Returns:
            bool: True if initialization succeeds; False if it fails or an exception occurs.
        """
        try:
    ...
    async def shutdown(self):
        ...
        After shutdown, the backend is marked as inactive. Errors during shutdown are logged.
        """
        try:
...
def health_check():
    ...
    - "status": Indicates if the Genesis backend is running ("healthy" or "unhealthy").
...
def chat_with_genesis():
    """
    Handles a chat request by forwarding the user's message, user ID, and optional context to the Genesis backend and returning the backend's response as JSON.
    ...
    Returns:
        Flask Response: JSON response from the Genesis backend, or an error message with the appropriate HTTP status code.
    """
    try:
        # Validate request
...
def get_status():
    ...
    Returns:
        Flask JSON response with the Genesis Layer's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def internal_error(error):
    """
    Handles uncaught server errors by returning a generic JSON error message with HTTP status code 500.
    
    Returns:
        tuple: JSON response containing error details and a 500 status code.
    """
    return jsonify({
        "error": "Internal server error",
