...
class GenesisAPI:
    ...
    async def startup(self):
        """
        Asynchronously starts the Genesis Layer backend and sets the running state.
    ...
    async def shutdown(self):
        """
        Asynchronously shuts down the Genesis Layer backend and marks the API as not running.
...
def run_async(coro):
    ...
    Intended for use within synchronous Flask route handlers to facilitate integration with asynchronous backend operations.
...
def get_status():
    ...
    Returns:
        A JSON response containing the Genesis Layer's status information, or an error message with HTTP 500 if retrieval fails.
    """
    try:
...
def get_genesis_profile():
    ...
    Returns:
        A JSON object containing the Genesis system's profile data, or an error message with HTTP 500 status if retrieval fails.
    """
    try:
        profile_data = {
...
def reset_session():
    ...
    Returns:
        Response: JSON indicating reset success with a timestamp, or an error message with HTTP 500 if the operation fails.
    """
    try:
...
def internal_error(error):
    ...
    Returns:
        tuple: JSON error message and HTTP 500 status code.
    """
    return jsonify({
        "error": "Internal server error",
...
def initialize_app():
    """
    Asynchronously starts the Genesis Layer backend before handling the first client request.
    """
...
def cleanup():
    """
    Asynchronously shuts down the Genesis Layer backend during application termination.
    """
