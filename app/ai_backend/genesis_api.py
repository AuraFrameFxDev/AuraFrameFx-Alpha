...
def run_async(coro):
    ...
    Intended for use in synchronous Flask route handlers to invoke asynchronous backend operations.
...
def get_status():
    ...
    Returns:
        JSON response with backend status information on success, or an error message with HTTP 500 status code if retrieval fails.
    """
    try:
...
def reset_session():
    ...
    Returns:
        Response: JSON containing the reset status, message, and timestamp on success, or an error message with HTTP 500 status if the reset fails.
    """
    try:
