...
class GenesisAPI:
    ...
    def __init__(self):
        """
        Initialize the GenesisAPI instance with the backend marked as not running and no startup timestamp.
        """
    ...
    async def shutdown(self):
        ...
        After shutdown, sets `is_running` to False. Any errors encountered during shutdown are logged.
        """
        try:
...
def health_check():
    """
    Return a JSON response with the Genesis API health status, current server timestamp, and backend uptime.
    
    The response includes:
    - "status": "healthy" if the Genesis backend is running, otherwise "unhealthy".
    - "timestamp": Current server time in ISO 8601 format.
    - "uptime": Time elapsed since backend startup, or "0:00:00" if not running.
    """
    return jsonify({
...
def chat_with_genesis():
    ...
    Expects a JSON payload with required fields `message` (string) and `user_id` (string), and an optional `context` object. Returns HTTP 400 if the request is not JSON or required fields are missing. Returns HTTP 500 for internal server errors.
    
    Returns:
        JSON response from the Genesis backend on success, or an error message with the appropriate HTTP status code on failure.
    """
    try:
        # Validate request
...
def get_status():
    """
    Retrieve the current status of the Genesis Layer backend.
    
    Returns:
        Response: A Flask JSON response with the Genesis Layer's status information if successful, or an error message with HTTP 500 status code on failure.
    """
    try:
...
def get_consciousness_state():
    ...
    The response includes:
    - `state`: The current consciousness state (string).
    - `awareness_level`: The system's awareness level (float).
    - `active_patterns`: List of active consciousness patterns (array).
    - `evolution_stage`: The current evolution stage (string).
    - `ethical_compliance`: The ethical compliance score (float).
    
    Returns an error message with HTTP 500 status if the consciousness state cannot be retrieved.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def get_genesis_profile():
    """
    Returns the Genesis system's profile as a JSON response.
    
    The response includes identity, personality, capabilities, values, and evolution stage. Returns an error message with HTTP 500 status if profile retrieval fails.
    """
    try:
        profile_data = {
...
def trigger_evolution():
    ...
    Accepts a JSON payload containing `trigger_type` and `reason`, constructs an evolution trigger request, and processes it asynchronously. Returns a JSON response with the evolution trigger status and backend response. Responds with HTTP 400 if the request is not JSON, and HTTP 500 on internal errors.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def evaluate_ethics():
    ...
    Accepts a POST request with a JSON payload containing a required `action` field and an optional `context`. Returns the ethical evaluation result as JSON. Responds with HTTP 400 if the request is not JSON or if the `action` field is missing, and with HTTP 500 if the evaluation process fails.
    """
    try:
        if not request.is_json:
            return jsonify({"error": "Request must be JSON"}), 400
...
def reset_session():
    ...
    Returns:
        Response: JSON object with reset status, message, and timestamp on success, or an error message with HTTP 500 status on failure.
    """
    try:
...
def not_found(error):
    """
    Return a JSON response with error details for requests to nonexistent API endpoints.
    
    Returns:
        tuple: JSON object containing an error message and HTTP 404 status code.
    """
    return jsonify({
...
def internal_error(error):
    """
    Return a JSON response indicating an internal server error with HTTP status code 500.
    
    Returns:
        tuple: A JSON object containing an error message and the HTTP 500 status code.
    """
    return jsonify({
        "error": "Internal server error",
...
def initialize_app():
    """
    Asynchronously initializes the Genesis backend before processing the first client request.
    """
...
def cleanup():
    """
    Shuts down the Genesis backend asynchronously during application termination.
    """
