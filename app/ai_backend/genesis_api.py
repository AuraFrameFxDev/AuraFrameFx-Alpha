...
def run_async(coro):
    ...
    Intended for use in synchronous contexts, such as Flask route handlers, to enable calling asynchronous backend functions.
...
def health_check():
    """
    Return a JSON object indicating the Genesis backend's health status, current server time, and uptime duration.
...
def get_consciousness_state():
    ...
    The response includes the system's consciousness state, awareness level, active patterns, evolution stage, and ethical compliance score. Returns HTTP 500 with an error message if retrieval fails.
    """
    try:
        status = run_async(get_genesis_status())
        consciousness_data = {
...
def reset_session():
    ...
    Returns:
        Response: A JSON object containing the reset status, message, and timestamp on success; returns an error message with HTTP 500 status on failure.
    """
    try:
