...
class GenesisAPI:
    ...
    async def shutdown(self):
        ...
        After completion, sets the `is_running` attribute to False. Any exceptions during shutdown are logged.
        """
        try:
...
def get_genesis_profile():
    ...
    Returns:
        A JSON object containing the Genesis profile attributes, or an error message with HTTP 500 status if retrieval fails.
    """
    try:
        profile_data = {
...
def reset_session():
    ...
    Returns:
        Response: JSON indicating the reset status, message, and timestamp on success, or an error message with HTTP 500 status if the reset fails.
    """
    try:
...
def internal_error(error):
    """
    Generate a JSON response for HTTP 500 errors with a generic error message.
...
def initialize_app():
    """
    Asynchronously starts the Genesis backend before handling the first client request.
    """
